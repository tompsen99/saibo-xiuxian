...
// Read JSON file (支持Supabase缓存)
function readJSON(filePath) {
  // 如果启用了Supabase，优先从缓存读取
  if (useSupabase) {
    const filename = require('path').basename(filePath);
    const cached = supabaseSync.readCache(filename);
...
const WORLD_DATA = {
  maps: {
    ...
    '江南水乡': {
      name: '江南水乡', realmRequirement: '筑基',
      description: '水乡泽国，鱼米之乡。小桥流水，烟雨朦胧。',
      rooms: {
        '水乡入口': { name: '水乡入口', description: '石桥横跨小河，两岸垂柳依依。', exits: { north: '渔村', east: '药田', south: '竹林' } },
        '渔村': { name: '渔村', description: '渔民们正在河边捕鱼，炊烟袅袅。', exits: { south: '水乡入口', north: '湖心岛' }, monsters: [{ name: '河蟹', hp: 80, maxHp: 80, attack: 15, defense: 8, exp: 35, silver: 12, dropRate: 0.2 }, { name: '水蛇', hp: 100, maxHp: 100, attack: 20, defense: 10, exp: 45, silver: 15, dropRate: 0.18 }] },
        '湖心岛': { name: '湖心岛', description: '湖中小岛，灵气浓郁。', exits: { south: '渔村' }, monsters: [{ name: '湖妖', hp: 200, maxHp: 200, attack: 35, defense: 18, exp: 80, silver: 30, dropRate: 0.15 }] },
        '药田': { name: '药田', description: '药田遍地，灵草丛生。采集灵草的绝佳之地。', exits: { west: '水乡入口', north: '绣坊' } },
        '绣坊': { name: '绣坊', description: '绣娘们正在刺绣，丝线闪烁灵光。', exits: { south: '药田' } }
      }
    },
    '沙漠戈壁': {
      name: '沙漠戈壁', realmRequirement: '金丹',
      description: '茫茫大漠，绿洲点缀。风沙漫天，烈日灼心。',
      rooms: {
        '沙漠入口': { name: '沙漠入口', description: '黄沙漫漫，远处可见绿洲的影子。', exits: { north: '绿洲', east: '风蚀石林' } },
        '绿洲': { name: '绿洲', description: '沙漠中的绿洲，泉水清澈。', exits: { south: '沙漠入口', north: '古城遗迹' }, monsters: [{ name: '沙蝎', hp: 250, maxHp: 250, attack: 45, defense: 20, exp: 120, silver: 40, dropRate: 0.15 }, { name: '沙暴虫', hp: 300, maxHp: 300, attack: 55, defense: 25, exp: 150, silver: 50, dropRate: 0.12 }] },
        '风蚀石林': { name: '风蚀石林', description: '奇石嶙峋，风声呼啸如鬼哭。', exits: { west: '沙漠入口', north: '古城遗迹' }, monsters: [{ name: '石灵', hp: 350, maxHp: 350, attack: 60, defense: 35, exp: 180, silver: 60, dropRate: 0.1 }] },
        '古城遗迹': { name: '古城遗迹', description: '被黄沙掩埋的古城，依稀可见昔日辉煌。', exits: { south: '绿洲', east: '地下密室' } },
        '地下密室': { name: '地下密室', description: '古城地下密室，墙壁刻满古老文字。', exits: { west: '古城遗迹' }, monsters: [{ name: '守墓傀儡', hp: 500, maxHp: 500, attack: 80, defense: 40, exp: 300, silver: 100, dropRate: 0.08 }] }
      }
    },
    '北境冰原': {
      name: '北境冰原', realmRequirement: '化神',
      description: '冰天雪地，极光闪烁。寒风刺骨，万物冰封。',
      rooms: {
        '冰原入口': { name: '冰原入口', description: '白雪皑皑，寒风凛冽。', exits: { north: '冰湖', east: '冰川' } },
        '冰湖': { name: '冰湖', description: '冰封的湖面，冰层下隐约有生物游动。', exits: { south: '冰原入口', north: '雪山之巅' }, monsters: [{ name: '冰狼', hp: 600, maxHp: 600, attack: 100, defense: 50, exp: 350, silver: 120, dropRate: 0.12 }, { name: '冰晶巨人', hp: 800, maxHp: 800, attack: 120, defense: 70, exp: 500, silver: 180, dropRate: 0.08 }] },
        '冰川': { name: '冰川', description: '千年冰川，冰层中封印着远古生物。', exits: { west: '冰原入口' }, monsters: [{ name: '冰龙', hp: 1000, maxHp: 1000, attack: 150, defense: 80, exp: 600, silver: 250, dropRate: 0.05 }] },
        '雪山之巅': { name: '雪山之巅', description: '雪山绝顶，极光在天空中舞动。', exits: { south: '冰湖' }, monsters: [{ name: '雪域冰凤', hp: 1500, maxHp: 1500, attack: 200, defense: 100, exp: 800, silver: 400, dropRate: 0.03 }] }
      }
    },
    '南疆密林': {
      name: '南疆密林', realmRequirement: '化神',
      description: '热带雨林，瘴气弥漫。毒虫遍地，危机四伏。',
      rooms: {
        '密林入口': { name: '密林入口', description: '热带雨林入口，潮湿闷热。', exits: { north: '毒沼', east: '藤蔓迷宫' } },
        '毒沼': { name: '毒沼', description: '沼泽地散发毒气，脚下泥泞不堪。', exits: { south: '密林入口', north: '蛊虫巢穴' }, monsters: [{ name: '毒蛙', hp: 400, maxHp: 400, attack: 70, defense: 30, exp: 200, silver: 70, dropRate: 0.18 }, { name: '沼泽蟒', hp: 500, maxHp: 500, attack: 90, defense: 40, exp: 280, silver: 90, dropRate: 0.12 }] },
        '藤蔓迷宫': { name: '藤蔓迷宫', description: '藤蔓交织成迷宫，方向难辨。', exits: { west: '密林入口' }, monsters: [{ name: '藤妖', hp: 450, maxHp: 450, attack: 80, defense: 35, exp: 250, silver: 80, dropRate: 0.15 }] },
        '蛊虫巢穴': { name: '蛊虫巢穴', description: '蛊虫的巢穴，到处是虫卵。', exits: { south: '毒沼', north: '巫蛊祭坛' }, monsters: [{ name: '蛊王', hp: 700, maxHp: 700, attack: 110, defense: 50, exp: 400, silver: 150, dropRate: 0.1 }] },
        '巫蛊祭坛': { name: '巫蛊祭坛', description: '古老的巫蛊祭坛，神秘力量涌动。', exits: { south: '蛊虫巢穴' }, monsters: [{ name: '巫蛊之主', hp: 1200, maxHp: 1200, attack: 180, defense: 90, exp: 700, silver: 350, dropRate: 0.05 }] }
      }
    },
    '海底龙宫': {
      name: '海底龙宫', realmRequirement: '合体',
      description: '海底世界，龙族居所。珊瑚林立，宝光闪烁。',
      rooms: {
        '浅海区': { name: '浅海区', description: '浅海珊瑚礁，鱼群穿梭。', exits: { north: '深海区' } },
        '深海区': { name: '深海区', description: '深海幽暗，巨大生物的影子掠过。', exits: { south: '浅海区', north: '龙宫大门' }, monsters: [{ name: '深海巨鲸', hp: 3000, maxHp: 3000, attack: 250, defense: 120, exp: 2000, silver: 800, dropRate: 0.08 }, { name: '海妖', hp: 2500, maxHp: 2500, attack: 200, defense: 100, exp: 1500, silver: 600, dropRate: 0.1 }] },
        '龙宫大门': { name: '龙宫大门', description: '金碧辉煌的龙宫大门，两条石龙盘旋。', exits: { south: '深海区', north: '龙宫大殿' } },
        '龙宫大殿': { name: '龙宫大殿', description: '龙宫大殿，珊瑚为柱，珍珠为灯。', exits: { south: '龙宫大门', north: '龙王宝库' }, monsters: [{ name: '龙宫守卫', hp: 4000, maxHp: 4000, attack: 300, defense: 150, exp: 3000, silver: 1200, dropRate: 0.05 }] },
        '龙王宝库': { name: '龙王宝库', description: '龙王的宝库，金银珠宝堆积如山。', exits: { south: '龙宫大殿' }, monsters: [{ name: '龙王', hp: 8000, maxHp: 8000, attack: 500, defense: 250, exp: 8000, silver: 5000, dropRate: 0.02 }] }
      }
    },
    '虚空裂缝': {
      name: '虚空裂缝', realmRequirement: '渡劫',
      description: '次元裂缝，时空扭曲。现实与虚无交织。',
      rooms: {
        '裂缝入口': { name: '裂缝入口', description: '空间裂缝，扭曲的光线令人眩晕。', exits: { north: '时空走廊' } },
        '时空走廊': { name: '时空走廊', description: '时空错乱的走廊，过去与未来交织。', exits: { south: '裂缝入口', north: '虚无之海' }, monsters: [{ name: '时空裂隙兽', hp: 5000, maxHp: 5000, attack: 400, defense: 200, exp: 5000, silver: 2000, dropRate: 0.05 }, { name: '虚空游荡者', hp: 4000, maxHp: 4000, attack: 350, defense: 180, exp: 4000, silver: 1500, dropRate: 0.08 }] },
        '虚无之海': { name: '虚无之海', description: '无尽的虚无，偶尔有数据碎片飘过。', exits: { south: '时空走廊', north: '虚空王座' }, monsters: [{ name: '虚无之主', hp: 10000, maxHp: 10000, attack: 600, defense: 300, exp: 10000, silver: 5000, dropRate: 0.02 }] },
        '虚空王座': { name: '虚空王座', description: '虚空的中心，一个巨大的王座漂浮在虚无中。', exits: { south: '虚无之海' }, monsters: [{ name: '虚空帝王', hp: 20000, maxHp: 20000, attack: 1000, defense: 500, exp: 25000, silver: 15000, dropRate: 0.01 }] }
      }
    }
  }
};
...
const PILLS_DATA = {
  ...
  // ===== 【新增】凡丹 =====
  str_pill: { id: 'str_pill', name: '力量丹', grade: '凡丹', effect: '力量+1(永久)', price: 80, recipe: { herb: 4, spirit_shard: 2, beast_core: 0 }, apply: (p) => { p.str = (p.str || 0) + 1; return '力量+1！'; } },
  dex_pill: { id: 'dex_pill', name: '敏捷丹', grade: '凡丹', effect: '敏捷+1(永久)', price: 80, recipe: { herb: 2, spirit_shard: 4, beast_core: 0 }, apply: (p) => { p.dex = (p.dex || 0) + 1; return '敏捷+1！'; } },
  con_pill: { id: 'con_pill', name: '体质丹', grade: '凡丹', effect: '体质+1(永久)', price: 80, recipe: { herb: 3, spirit_shard: 3, beast_core: 0 }, apply: (p) => { p.con = (p.con || 0) + 1; return '体质+1！'; } },
  wis_pill: { id: 'wis_pill', name: '悟性丹', grade: '凡丹', effect: '悟性+1(永久)', price: 80, recipe: { herb: 2, spirit_shard: 2, beast_core: 2 }, apply: (p) => { p.wis = (p.wis || 0) + 1; return '悟性+1！'; } },
  // ===== 【新增】玄丹 =====
  speed_pill: { id: 'speed_pill', name: '疾风丹', grade: '玄丹', effect: '速度+10(30分钟)', price: 60, recipe: { herb: 3, spirit_shard: 3, beast_core: 1 }, apply: (p) => { p.speedBuff = Date.now() + 1800000; return '速度+10，持续30分钟！'; } },
  crit_pill: { id: 'crit_pill', name: '暴击丹', grade: '玄丹', effect: '暴击+5%(30分钟)', price: 80, recipe: { herb: 4, spirit_shard: 3, beast_core: 1 }, apply: (p) => { p.critBuff = Date.now() + 1800000; return '暴击率+5%，持续30分钟！'; } },
  dao_pill: { id: 'dao_pill', name: '道心丹', grade: '地丹', effect: '道心+50', price: 250, recipe: { herb: 6, spirit_shard: 4, beast_core: 3 }, apply: (p) => { p.daoHeart = Math.min(2000, (p.daoHeart || 500) + 50); return `道心+50！当前道心: ${p.daoHeart}`; } },
  lifespan_pill: { id: 'lifespan_pill', name: '延寿丹', grade: '地丹', effect: '寿命+10', price: 200, recipe: { herb: 5, spirit_shard: 5, beast_core: 2 }, apply: (p) => { p.lifespan = (p.lifespan || 100) + 10; return `寿命+10！当前寿命: ${p.lifespan}`; } },
  breakthrough_pill: { id: 'breakthrough_pill', name: '突破丹', grade: '地丹', effect: '突破率+20%', price: 150, recipe: { herb: 6, spirit_shard: 3, beast_core: 2 }, apply: (p) => { p.breakthroughBuff = true; return '下次突破成功率+20%！'; } },
  attribute_reset: { id: 'attribute_reset', name: '洗髓丹', grade: '天丹', effect: '重置所有属性点', price: 500, recipe: { herb: 10, spirit_shard: 10, beast_core: 5 }, apply: (p) => { p.str = 10; p.dex = 10; p.con = 10; p.wis = 10; return '属性已重置！'; } },
  skill_learn_pill: { id: 'skill_learn_pill', name: '悟道丹', grade: '天丹', effect: '学会一个随机功法', price: 400, recipe: { herb: 8, spirit_shard: 8, beast_core: 4 }, apply: (p) => { return '请使用后查看效果。'; } },
  ...
  // ===== 【新增】更多丹药 =====
  mini_hp_pill: { id: 'mini_hp_pill', name: '小回血丹', grade: '凡丹', effect: '恢复25HP', price: 5, recipe: { herb: 1, spirit_shard: 0, beast_core: 0 }, apply: (p) => { const r = Math.min(25, (p.maxHp||100)-(p.hp||0)); p.hp = Math.min(p.maxHp||100, (p.hp||0)+25); return `恢复了${r}点HP`; } },
  mini_spirit_pill: { id: 'mini_spirit_pill', name: '小回灵丹', grade: '凡丹', effect: '恢复15灵力', price: 8, recipe: { herb: 0, spirit_shard: 1, beast_core: 0 }, apply: (p) => { const r = Math.min(15, (p.maxSpirit||50)-(p.spirit||0)); p.spirit = Math.min(p.maxSpirit||50, (p.spirit||0)+15); return `恢复了${r}点灵力`; } },
  stamina_regen_pill: { id: 'stamina_regen_pill', name: '回体力丹', grade: '凡丹', effect: '恢复30体力', price: 12, recipe: { herb: 2, spirit_shard: 0, beast_core: 0 }, apply: (p) => { const r = Math.min(30, (p.maxStamina||100)-(p.stamina||0)); p.stamina = Math.min(p.maxStamina||100, (p.stamina||0)+30); return `恢复了${r}点体力`; } },
  berserker_pill: { id: 'berserker_pill', name: '狂战丹', grade: '玄丹', effect: '攻击+30%但防御-10%(30分钟)', price: 90, recipe: { herb: 4, spirit_shard: 2, beast_core: 2 }, apply: (p) => { p.berserkerBuff = Date.now() + 1800000; return '攻击+30%，防御-10%，持续30分钟！'; } },
  iron_skin_pill: { id: 'iron_skin_pill', name: '铁皮丹', grade: '玄丹', effect: '防御+30%但速度-10%(30分钟)', price: 90, recipe: { herb: 3, spirit_shard: 3, beast_core: 2 }, apply: (p) => { p.ironSkinBuff = Date.now() + 1800000; return '防御+30%，速度-10%，持续30分钟！'; } },
  swift_pill: { id: 'swift_pill', name: '疾速丹', grade: '玄丹', effect: '速度+20%(30分钟)', price: 80, recipe: { herb: 3, spirit_shard: 2, beast_core: 1 }, apply: (p) => { p.swiftBuff = Date.now() + 1800000; return '速度+20%，持续30分钟！'; } },
  fortune_pill: { id: 'fortune_pill', name: '福运丹', grade: '地丹', effect: '掉率+30%(1小时)', price: 300, recipe: { herb: 6, spirit_shard: 5, beast_core: 3 }, apply: (p) => { p.fortuneBuff = Date.now() + 3600000; return '掉率+30%，持续1小时！'; } },
  cultivation_pill: { id: 'cultivation_pill', name: '修炼丹', grade: '地丹', effect: '挂机经验+50%(1小时)', price: 250, recipe: { herb: 5, spirit_shard: 4, beast_core: 2 }, apply: (p) => { p.cultivationBuff = Date.now() + 3600000; return '挂机经验+50%，持续1小时！'; } },
  silver_pill: { id: 'silver_pill', name: '聚灵丹', grade: '地丹', effect: '灵石获取+50%(1小时)', price: 200, recipe: { herb: 4, spirit_shard: 4, beast_core: 2 }, apply: (p) => { p.silverBuff = Date.now() + 3600000; return '灵石获取+50%，持续1小时！'; } },
  full_restore_pill: { id: 'full_restore_pill', name: '天元丹', grade: '天丹', effect: '恢复全部HP/灵力/体力', price: 400, recipe: { herb: 8, spirit_shard: 8, beast_core: 4 }, apply: (p) => { p.hp = p.maxHp||100; p.spirit = p.maxSpirit||50; p.stamina = p.maxStamina||100; return '全部恢复！'; } },
  exp_surge_pill: { id: 'exp_surge_pill', name: '经验爆发丹', grade: '天丹', effect: '下次战斗经验x3', price: 350, recipe: { herb: 7, spirit_shard: 6, beast_core: 3 }, apply: (p) => { p.expSurgeBuff = true; return '下次战斗经验x3！'; } },
  silver_surge_pill: { id: 'silver_surge_pill', name: '灵石爆发丹', grade: '天丹', effect: '下次战斗灵石x3', price: 350, recipe: { herb: 6, spirit_shard: 7, beast_core: 3 }, apply: (p) => { p.silverSurgeBuff = true; return '下次战斗灵石x3！'; } },
  god_hp_pill: { id: 'god_hp_pill', name: '仙血丹', grade: '仙丹', effect: 'HP上限永久+50', price: 1500, recipe: { herb: 15, spirit_shard: 12, beast_core: 6 }, apply: (p) => { p.maxHp = (p.maxHp||100) + 50; p.hp = p.maxHp; return 'HP上限永久+50！'; } },
  god_spirit_pill: { id: 'god_spirit_pill', name: '仙灵丹', grade: '仙丹', effect: '灵力上限永久+30', price: 1500, recipe: { herb: 12, spirit_shard: 15, beast_core: 6 }, apply: (p) => { p.maxSpirit = (p.maxSpirit||50) + 30; p.spirit = p.maxSpirit; return '灵力上限永久+30！'; } }
};
// ===== ALCHEMY MATERIALS (炼丹材料) =====
const MATERIALS_DATA = {
  // ===== 基础材料(15种) =====
  herb: { id: 'herb', name: '灵草', grade: '普通', desc: '常见灵草，炼丹基础材料。', dropChance: 0.30 },
  spirit_shard: { id: 'spirit_shard', name: '灵石碎片', grade: '稀有', desc: '蕴含灵力的碎片。', dropChance: 0.15 },
  beast_core: { id: 'beast_core', name: '妖兽内丹', grade: '珍贵', desc: '妖兽体内的内丹。', dropChance: 0.05 },
  mushroom: { id: 'mushroom', name: '灵芝', grade: '普通', desc: '灵芝，炼丹材料。', dropChance: 0.25 },
  flower: { id: 'flower', name: '灵花', grade: '普通', desc: '灵花，炼丹材料。', dropChance: 0.20 },
  ore_dust: { id: 'ore_dust', name: '矿粉', grade: '普通', desc: '矿石粉末，锻造材料。', dropChance: 0.20 },
  spider_silk: { id: 'spider_silk', name: '蛛丝', grade: '稀有', desc: '蜘蛛丝，防具材料。', dropChance: 0.12 },
  wolf_fang: { id: 'wolf_fang', name: '狼牙', grade: '稀有', desc: '狼牙，武器材料。', dropChance: 0.12 },
  snake_gall: { id: 'snake_gall', name: '蛇胆', grade: '稀有', desc: '蛇胆，丹药材料。', dropChance: 0.10 },
  tiger_bone: { id: 'tiger_bone', name: '虎骨', grade: '珍贵', desc: '虎骨，高级丹药材料。', dropChance: 0.08 },
  dragon_blood: { id: 'dragon_blood', name: '龙血', grade: '传说', desc: '龙血，天丹材料。', dropChance: 0.03 },
  phoenix_tear: { id: 'phoenix_tear', name: '凤泪', grade: '传说', desc: '凤凰之泪，天丹材料。', dropChance: 0.03 },
  void_fragment: { id: 'void_fragment', name: '虚空碎片', grade: '神话', desc: '虚空碎片，仙丹材料。', dropChance: 0.01 },
  enhance_stone: { id: 'enhance_stone', name: '强化石', grade: '稀有', desc: '装备强化材料。', dropChance: 0.10 },
  refine_stone: { id: 'refine_stone', name: '精炼石', grade: '珍贵', desc: '装备精炼材料。', dropChance: 0.05 },
  // ===== 矿石类(15种) =====
  copper_ore: { id: 'copper_ore', name: '铜矿', grade: '普通', desc: '铜矿石，基础锻造材料。', dropChance: 0.25 },
  iron_ore: { id: 'iron_ore', name: '铁矿', grade: '普通', desc: '铁矿石，锻造材料。', dropChance: 0.20 },
  silver_ore: { id: 'silver_ore', name: '银矿', grade: '稀有', desc: '银矿石，高级锻造材料。', dropChance: 0.12 },
  gold_ore: { id: 'gold_ore', name: '金矿', grade: '稀有', desc: '金矿石，稀有锻造材料。', dropChance: 0.08 },
  mithril_ore: { id: 'mithril_ore', name: '秘银矿', grade: '珍贵', desc: '秘银矿石，传说锻造材料。', dropChance: 0.05 },
  adamantium_ore: { id: 'adamantium_ore', name: '精金矿', grade: '珍贵', desc: '精金矿石，最强锻造材料。', dropChance: 0.03 },
  fire_crystal: { id: 'fire_crystal', name: '火晶石', grade: '稀有', desc: '火属性晶石，火系装备材料。', dropChance: 0.10 },
  ice_crystal: { id: 'ice_crystal', name: '冰晶石', grade: '稀有', desc: '冰属性晶石，冰系装备材料。', dropChance: 0.10 },
  thunder_stone: { id: 'thunder_stone', name: '雷石', grade: '稀有', desc: '雷属性石头，雷系装备材料。', dropChance: 0.10 },
  wind_gem: { id: 'wind_gem', name: '风灵石', grade: '稀有', desc: '风属性宝石，风系装备材料。', dropChance: 0.10 },
  earth_core: { id: 'earth_core', name: '地心石', grade: '稀有', desc: '土属性核心，土系装备材料。', dropChance: 0.10 },
  dark_essence: { id: 'dark_essence', name: '暗影精华', grade: '珍贵', desc: '暗属性精华，暗系装备材料。', dropChance: 0.08 },
  light_essence: { id: 'light_essence', name: '圣光精华', grade: '珍贵', desc: '光属性精华，光系装备材料。', dropChance: 0.08 },
  star_metal: { id: 'star_metal', name: '星辰铁', grade: '传说', desc: '陨石中的金属，传说锻造材料。', dropChance: 0.03 },
  moon_stone: { id: 'moon_stone', name: '月华石', grade: '传说', desc: '吸收月光的宝石。', dropChance: 0.03 },
  // ===== 草药类(15种) =====
  ginseng: { id: 'ginseng', name: '人参', grade: '普通', desc: '百年人参，补气药材。', dropChance: 0.22 },
  lingzhi: { id: 'lingzhi', name: '灵芝草', grade: '普通', desc: '灵芝草，炼丹常用。', dropChance: 0.20 },
  snow_lotus: { id: 'snow_lotus', name: '雪莲', grade: '稀有', desc: '高山雪莲，珍贵药材。', dropChance: 0.10 },
  blood_grass: { id: 'blood_grass', name: '血草', grade: '稀有', desc: '红色灵草，补血药材。', dropChance: 0.12 },
  soul_flower: { id: 'soul_flower', name: '魂花', grade: '稀有', desc: '灵魂之花，恢复灵力。', dropChance: 0.10 },
  dream_grass: { id: 'dream_grass', name: '梦草', grade: '稀有', desc: '梦幻灵草，安神药材。', dropChance: 0.10 },
  poison_ivy: { id: 'poison_ivy', name: '毒藤', grade: '普通', desc: '有毒藤蔓，毒系材料。', dropChance: 0.18 },
  phoenix_grass: { id: 'phoenix_grass', name: '凤凰草', grade: '珍贵', desc: '浴火重生的灵草。', dropChance: 0.05 },
  dragon_root: { id: 'dragon_root', name: '龙根', grade: '珍贵', desc: '龙脉深处的灵根。', dropChance: 0.05 },
  star_flower: { id: 'star_flower', name: '星辰花', grade: '珍贵', desc: '吸收星光的花朵。', dropChance: 0.05 },
  moonlight_dew: { id: 'moonlight_dew', name: '月华露', grade: '稀有', desc: '月光凝结的露水。', dropChance: 0.08 },
  starlight_dust: { id: 'starlight_dust', name: '星辰砂', grade: '珍贵', desc: '星辰陨落的粉尘。', dropChance: 0.05 },
  void_moss: { id: 'void_moss', name: '虚空苔', grade: '传说', desc: '虚空裂缝中生长的苔藓。', dropChance: 0.02 },
  chaos_seed: { id: 'chaos_seed', name: '混沌种子', grade: '神话', desc: '混沌之力凝聚的种子。', dropChance: 0.01 },
  genesis_leaf: { id: 'genesis_leaf', name: '创世之叶', grade: '神话', desc: '创世神树的叶子。', dropChance: 0.01 },
  // ===== 兽材类(15种) =====
  rabbit_foot: { id: 'rabbit_foot', name: '兔脚', grade: '普通', desc: '野兔脚，幸运符材料。', dropChance: 0.30 },
  feather: { id: 'feather', name: '羽毛', grade: '普通', desc: '鸟类羽毛，轻甲材料。', dropChance: 0.28 },
  bone: { id: 'bone', name: '兽骨', grade: '普通', desc: '野兽骨骼，武器材料。', dropChance: 0.25 },
  leather: { id: 'leather', name: '兽皮', grade: '普通', desc: '野兽皮革，防具材料。', dropChance: 0.22 },
  claw: { id: 'claw', name: '兽爪', grade: '普通', desc: '锋利的兽爪，武器材料。', dropChance: 0.20 },
  bear_paw: { id: 'bear_paw', name: '熊掌', grade: '稀有', desc: '黑熊之掌，珍贵材料。', dropChance: 0.12 },
  scorpion_tail: { id: 'scorpion_tail', name: '蝎尾', grade: '稀有', desc: '毒蝎之尾，毒系材料。', dropChance: 0.12 },
  turtle_shell: { id: 'turtle_shell', name: '龟壳', grade: '稀有', desc: '灵龟之壳，防御材料。', dropChance: 0.10 },
  eagle_eye: { id: 'eagle_eye', name: '鹰眼', grade: '稀有', desc: '灵鹰之眼，饰品材料。', dropChance: 0.10 },
  lion_heart: { id: 'lion_heart', name: '狮心', grade: '珍贵', desc: '雄狮之心，勇气材料。', dropChance: 0.08 },
  dragon_scale_m: { id: 'dragon_scale_m', name: '龙鳞', grade: '传说', desc: '龙族之鳞，传说防具材料。', dropChance: 0.03 },
  phoenix_feather_m: { id: 'phoenix_feather_m', name: '凤羽', grade: '传说', desc: '凤凰之羽，传说饰品材料。', dropChance: 0.03 },
  qilin_horn: { id: 'qilin_horn', name: '麒麟角', grade: '传说', desc: '麒麟之角，传说武器材料。', dropChance: 0.02 },
  white_tiger_fang: { id: 'white_tiger_fang', name: '白虎牙', grade: '传说', desc: '白虎之牙，传说武器材料。', dropChance: 0.02 },
  black_tortoise_shell: { id: 'black_tortoise_shell', name: '玄武甲', grade: '传说', desc: '玄武之甲，传说防具材料。', dropChance: 0.02 },
  // ===== 特殊材料(25种) =====
  wood: { id: 'wood', name: '木材', grade: '普通', desc: '普通木材，基础制作材料。', dropChance: 0.35 },
  cloth: { id: 'cloth', name: '布料', grade: '普通', desc: '普通布料，防具材料。', dropChance: 0.30 },
  rope: { id: 'rope', name: '绳索', grade: '普通', desc: '结实的绳索。', dropChance: 0.25 },
  glass: { id: 'glass', name: '琉璃', grade: '稀有', desc: '彩色琉璃，饰品材料。', dropChance: 0.12 },
  jade: { id: 'jade', name: '玉石', grade: '稀有', desc: '温润玉石，饰品材料。', dropChance: 0.10 },
  pearl: { id: 'pearl', name: '珍珠', grade: '稀有', desc: '海底珍珠，饰品材料。', dropChance: 0.08 },
  amber: { id: 'amber', name: '琥珀', grade: '稀有', desc: '远古琥珀，法宝材料。', dropChance: 0.08 },
  coral: { id: 'coral', name: '珊瑚', grade: '稀有', desc: '海底珊瑚，法宝材料。', dropChance: 0.08 },
  silk: { id: 'silk', name: '天蚕丝', grade: '珍贵', desc: '天蚕吐丝，高级防具材料。', dropChance: 0.05 },
  ink: { id: 'ink', name: '灵墨', grade: '稀有', desc: '灵墨，符箓材料。', dropChance: 0.10 },
  paper: { id: 'paper', name: '灵纸', grade: '稀有', desc: '灵纸，符箓材料。', dropChance: 0.10 },
  rune_stone: { id: 'rune_stone', name: '符文石', grade: '珍贵', desc: '刻有符文的石头，法宝材料。', dropChance: 0.05 },
  soul_gem: { id: 'soul_gem', name: '灵魂宝石', grade: '珍贵', desc: '蕴含灵魂的宝石。', dropChance: 0.05 },
  time_sand: { id: 'time_sand', name: '时间之砂', grade: '传说', desc: '时间凝固的砂砾。', dropChance: 0.02 },
  fate_thread: { id: 'fate_thread', name: '命运之线', grade: '传说', desc: '命运女神的丝线。', dropChance: 0.02 },
  space_shard: { id: 'space_shard', name: '空间碎片', grade: '传说', desc: '空间破裂的碎片。', dropChance: 0.02 },
  chaos_stone: { id: 'chaos_stone', name: '混沌石', grade: '神话', desc: '混沌之力凝聚的石头。', dropChance: 0.01 },
  origin_dust: { id: 'origin_dust', name: '本源之尘', grade: '神话', desc: '世界本源的尘埃。', dropChance: 0.01 },
  eternal_ice: { id: 'eternal_ice', name: '永恒之冰', grade: '传说', desc: '永不融化的冰晶。', dropChance: 0.02 },
  inferno_core: { id: 'inferno_core', name: '炼狱核心', grade: '传说', desc: '炼狱之火的精华。', dropChance: 0.02 },
  void_essence: { id: 'void_essence', name: '虚空精华', grade: '神话', desc: '虚空之力的精华。', dropChance: 0.01 },
  code_fragment: { id: 'code_fragment', name: '代码碎片', grade: '神话', desc: '世界源代码的碎片。', dropChance: 0.01 },
  memory_crystal: { id: 'memory_crystal', name: '记忆水晶', grade: '神话', desc: '存储记忆的水晶。', dropChance: 0.01 },
  life_seed: { id: 'life_seed', name: '生命之种', grade: '神话', desc: '蕴含生命之力的种子。', dropChance: 0.01 },
  death_essence: { id: 'death_essence', name: '死亡精华', grade: '神话', desc: '死亡之力的凝聚。', dropChance: 0.01 }
};
...
const EQUIPMENT_DATA = {
  ...
  // ===== 【新增】武器 =====
  bamboo_spear: { id: 'bamboo_spear', name: '竹枪', slot: 'weapon', attack: 10, speed: 2, price: 60, description: '竹制长枪，轻便灵活。' },
  hunters_bow: { id: 'hunters_bow', name: '猎弓', slot: 'weapon', attack: 14, speed: 3, critRate: 0.02, price: 120, description: '猎人专用弓，精准致命。' },
  iron_staff: { id: 'iron_staff', name: '铁杖', slot: 'weapon', attack: 8, spirit: 25, price: 100, description: '蕴含灵力的铁杖。' },
  steel_staff: { id: 'steel_staff', name: '钢杖', slot: 'weapon', attack: 15, spirit: 50, price: 300, description: '精钢锻造的法杖。' },
  shadow_blade: { id: 'shadow_blade', name: '暗影刃', slot: 'weapon', attack: 28, speed: 8, critRate: 0.05, price: 600, description: '暗影中锻造的利刃。' },
  thunder_hammer: { id: 'thunder_hammer', name: '雷神锤', slot: 'weapon', attack: 45, defense: 5, speed: -5, critRate: 0.08, price: 1200, description: '雷神遗落的战锤。' },
  ice_crystal_sword: { id: 'ice_crystal_sword', name: '冰晶剑', slot: 'weapon', attack: 40, speed: 5, critRate: 0.05, price: 1000, description: '冰晶铸就的宝剑。' },
  dragon_fang_spear: { id: 'dragon_fang_spear', name: '龙牙枪', slot: 'weapon', attack: 60, speed: 10, critRate: 0.10, price: 3000, description: '龙牙打造的神枪。' },
  phoenix_fan: { id: 'phoenix_fan', name: '凤羽扇', slot: 'weapon', attack: 55, speed: 12, critRate: 0.08, price: 2800, description: '凤羽编织的宝扇。' },
  void_blade: { id: 'void_blade', name: '虚空刃', slot: 'weapon', attack: 80, speed: 15, critRate: 0.15, price: 8000, description: '虚空之力凝聚的神兵。' },
  // ===== 【新增】护甲 =====
  bamboo_armor: { id: 'bamboo_armor', name: '竹甲', slot: 'armor', defense: 5, maxHp: 30, dodge: 2, price: 40, description: '竹片编织的轻甲。' },
  hunters_cloak: { id: 'hunters_cloak', name: '猎人斗篷', slot: 'armor', defense: 6, maxHp: 40, dodge: 5, price: 80, description: '猎人专用斗篷。' },
  steel_armor: { id: 'steel_armor', name: '钢甲', slot: 'armor', defense: 22, maxHp: 150, price: 400, description: '精钢锻造的重甲。' },
  silver_robe: { id: 'silver_robe', name: '银丝袍', slot: 'armor', defense: 18, maxHp: 120, spirit: 30, dodge: 3, price: 350, description: '银丝编织的法袍。' },
  fire_armor: { id: 'fire_armor', name: '烈焰甲', slot: 'armor', defense: 30, maxHp: 200, price: 800, description: '火焰淬炼的战甲。' },
  ice_robe: { id: 'ice_robe', name: '寒冰袍', slot: 'armor', defense: 25, maxHp: 180, dodge: 5, price: 750, description: '寒冰凝结的法袍。' },
  dragon_scale: { id: 'dragon_scale', name: '龙鳞甲', slot: 'armor', defense: 45, maxHp: 350, price: 2500, description: '龙鳞打造的神甲。' },
  phoenix_cloak: { id: 'phoenix_cloak', name: '凤凰斗篷', slot: 'armor', defense: 40, maxHp: 300, dodge: 10, price: 3000, description: '凤凰羽毛编织的斗篷。' },
  void_armor: { id: 'void_armor', name: '虚空战甲', slot: 'armor', defense: 60, maxHp: 500, price: 7000, description: '虚空之力凝聚的神甲。' },
  // ===== 【新增】头盔 =====
  leather_cap: { id: 'leather_cap', name: '皮帽', slot: 'helmet', defense: 4, maxHp: 20, price: 35, description: '皮革制成的帽子。' },
  iron_helm: { id: 'iron_helm', name: '铁盔', slot: 'helmet', defense: 10, maxHp: 60, price: 150, description: '铁制头盔。' },
  thunder_crown: { id: 'thunder_crown', name: '雷霆冠', slot: 'helmet', defense: 30, maxHp: 200, spirit: 80, critRate: 0.05, price: 2000, description: '雷霆之力凝聚的王冠。' },
  void_crown: { id: 'void_crown', name: '虚空冠', slot: 'helmet', defense: 40, maxHp: 300, spirit: 120, price: 5000, description: '虚空之力凝聚的皇冠。' },
  // ===== 【新增】戒指 =====
  jade_ring: { id: 'jade_ring', name: '玉戒指', slot: 'ring', attack: 8, defense: 8, daoHeart: 10, price: 180, description: '翠玉雕琢的戒指。' },
  destiny_ring: { id: 'destiny_ring', name: '命运之戒', slot: 'ring', attack: 35, defense: 30, critRate: 0.10, price: 5000, description: '掌控命运的神秘戒指。' },
  // ===== 【新增】更多武器 =====
  wooden_staff: { id: 'wooden_staff', name: '木杖', slot: 'weapon', attack: 3, spirit: 15, price: 25, description: '蕴含微弱灵力的木杖。' },
  bone_club: { id: 'bone_club', name: '骨棒', slot: 'weapon', attack: 8, price: 40, description: '兽骨制成的粗犷武器。' },
  jade_sword: { id: 'jade_sword', name: '玉剑', slot: 'weapon', attack: 18, spirit: 20, price: 250, description: '玉石雕琢的灵剑。' },
  wind_blade: { id: 'wind_blade', name: '风刃', slot: 'weapon', attack: 25, speed: 10, price: 400, description: '风属性利刃，出手极快。' },
  earth_hammer: { id: 'earth_hammer', name: '大地之锤', slot: 'weapon', attack: 35, defense: 10, speed: -5, price: 500, description: '沉重的战锤，一击碎石。' },
  water_ribbon: { id: 'water_ribbon', name: '水灵绸', slot: 'weapon', attack: 20, spirit: 40, price: 450, description: '水灵力凝聚的绸带。' },
  holy_staff: { id: 'holy_staff', name: '圣光杖', slot: 'weapon', attack: 15, spirit: 60, price: 600, description: '圣光凝聚的法杖。' },
  dark_scythe: { id: 'dark_scythe', name: '暗影镰', slot: 'weapon', attack: 50, critRate: 0.12, price: 800, description: '暗影之力凝聚的镰刀。' },
  sky_bow: { id: 'sky_bow', name: '天穹弓', slot: 'weapon', attack: 45, speed: 8, critRate: 0.08, price: 700, description: '射天之弓，百步穿杨。' },
  chaos_sword: { id: 'chaos_sword', name: '混沌剑', slot: 'weapon', attack: 70, spirit: 30, critRate: 0.10, price: 2000, description: '混沌之力铸就的神剑。' },
  // ===== 【新增】更多护甲 =====
  fur_armor: { id: 'fur_armor', name: '毛皮甲', slot: 'armor', defense: 4, maxHp: 25, price: 30, description: '兽毛皮制的简易护甲。' },
  scale_armor: { id: 'scale_armor', name: '鳞甲', slot: 'armor', defense: 12, maxHp: 80, price: 150, description: '鱼鳞编织的轻甲。' },
  mystic_robe: { id: 'mystic_robe', name: '玄丝袍', slot: 'armor', defense: 16, maxHp: 100, spirit: 25, price: 280, description: '玄蚕丝编织的法袍。' },
  heavy_plate: { id: 'heavy_plate', name: '重铠', slot: 'armor', defense: 28, maxHp: 180, speed: -3, price: 450, description: '厚重的全身铠甲。' },
  shadow_cloak: { id: 'shadow_cloak', name: '暗影斗篷', slot: 'armor', defense: 20, maxHp: 120, dodge: 8, price: 380, description: '暗影编织的斗篷，提升闪避。' },
  holy_armor: { id: 'holy_armor', name: '圣光铠', slot: 'armor', defense: 35, maxHp: 250, price: 900, description: '圣光祝福的铠甲。' },
  // ===== 【新增】更多头盔 =====
  fur_cap: { id: 'fur_cap', name: '毛皮帽', slot: 'helmet', defense: 3, maxHp: 15, price: 20, description: '兽毛皮制的帽子。' },
  bronze_helm: { id: 'bronze_helm', name: '青铜盔', slot: 'helmet', defense: 8, maxHp: 45, price: 100, description: '青铜锻造的头盔。' },
  mystic_hood: { id: 'mystic_hood', name: '玄丝兜帽', slot: 'helmet', defense: 12, maxHp: 60, spirit: 20, price: 200, description: '玄蚕丝编织的兜帽。' },
  holy_crown: { id: 'holy_crown', name: '圣光冠', slot: 'helmet', defense: 25, maxHp: 150, spirit: 50, price: 800, description: '圣光祝福的王冠。' },
  // ===== 【新增】更多戒指 =====
  copper_ring: { id: 'copper_ring', name: '铜戒指', slot: 'ring', attack: 3, defense: 3, price: 25, description: '铜制戒指，基础属性。' },
  iron_ring: { id: 'iron_ring', name: '铁戒指', slot: 'ring', attack: 6, defense: 6, price: 60, description: '铁制戒指，略优于铜。' },
  spirit_ring: { id: 'spirit_ring', name: '灵力戒', slot: 'ring', spirit: 30, price: 120, description: '蕴含灵力的戒指。' },
  blood_ring: { id: 'blood_ring', name: '血玉戒', slot: 'ring', attack: 12, defense: 8, price: 250, description: '血红色的玉戒，蕴含杀气。' },
  // ===== 【新增】更多饰品 =====
  wooden_amulet: { id: 'wooden_amulet', name: '木护身符', slot: 'accessory', maxHp: 20, price: 15, description: '木质护身符，微增生命。' },
  stone_amulet: { id: 'stone_amulet', name: '石护身符', slot: 'accessory', defense: 5, price: 30, description: '石质护身符，微增防御。' },
  crystal_pendant: { id: 'crystal_pendant', name: '水晶坠', slot: 'accessory', spirit: 25, price: 80, description: '水晶吊坠，增加灵力。' },
  lucky_charm: { id: 'lucky_charm', name: '幸运符', slot: 'accessory', price: 100, description: '据说能带来好运的护身符。' },
  exp_talisman: { id: 'exp_talisman', name: '经验符', slot: 'accessory', price: 150, description: '佩戴后经验获取增加10%。' }
};
...
const TREASURES_DATA = {
  ...
  // ===== 【新增】高级法宝 =====
  golden_pagoda: { id: 'golden_pagoda', name: '金光宝塔', grade: '灵器', type: 'defense', effect: { shield: 200, defense: 15 }, price: 1000, description: '金光闪闪的宝塔，可凝聚强力护盾。' },
  dragon_mirror: { id: 'dragon_mirror', name: '龙纹宝镜', grade: '灵器', type: 'utility', effect: { spirit: 50, wisdom: 10 }, price: 1200, description: '刻有龙纹的宝镜，增加灵力和悟性。' },
  phoenix_lamp: { id: 'phoenix_lamp', name: '凤凰神灯', grade: '灵器', type: 'heal', effect: { hpRegen: 20, spirit: 30 }, price: 1500, description: '凤凰羽毛点燃的神灯，持续恢复生命。' },
  void_bell: { id: 'void_bell', name: '虚空铃铛', grade: '仙器', type: 'utility', effect: { dodge: 15, speed: 20 }, price: 2500, description: '虚空之力凝聚的铃铛，提升闪避和速度。' },
  chaos_seal: { id: 'chaos_seal', name: '混沌神印', grade: '仙器', type: 'attack', effect: { attack: 30, critRate: 0.10, critDamage: 0.20 }, price: 3000, description: '混沌之力凝聚的神印，大幅提升攻击和暴击。' }
};
...
const QUESTS_DATA = {
  ...
  // ===== 【新增】新手村主线任务 =====
  xc_01: { id: 'xc_01', name: '初到村庄', type: 'main', description: '与村长对话，了解村庄', requirement: { type: 'talk', target: '村长' }, reward: { exp: 50 }, prereq: null },
  xc_02: { id: 'xc_02', name: '竹林试炼', type: 'main', description: '击杀3只野兔', requirement: { type: 'kill_any', target: 3 }, reward: { exp: 100, pills: { hp_pill: 3 } }, prereq: 'xc_01' },
  xc_03: { id: 'xc_03', name: '药铺帮手', type: 'main', description: '采集5株灵草', requirement: { type: 'gather', target: 5 }, reward: { exp: 80, silver: 50 }, prereq: 'xc_01' },
  xc_04: { id: 'xc_04', name: '铁匠的委托', type: 'main', description: '击杀5只野鸡', requirement: { type: 'kill_any', target: 5 }, reward: { exp: 100, equipment: 'iron_sword' }, prereq: 'xc_02' },
  xc_05: { id: 'xc_05', name: '客栈风波', type: 'main', description: '赶走闹事的醉汉', requirement: { type: 'kill_any', target: 1 }, reward: { exp: 200 }, prereq: 'xc_01' },
  xc_06: { id: 'xc_06', name: '集市纠纷', type: 'main', description: '帮商贩找回货物', requirement: { type: 'explore', target: '竹林' }, reward: { exp: 150, silver: 100 }, prereq: 'xc_05' },
  xc_07: { id: 'xc_07', name: '药铺秘方', type: 'main', description: '帮药铺老板试制新丹药', requirement: { type: 'gather', target: 3 }, reward: { exp: 100, pills: { adv_hp_pill: 3 } }, prereq: 'xc_03' },
  xc_08: { id: 'xc_08', name: '铁匠的烦恼', type: 'main', description: '收集10块铁矿', requirement: { type: 'gather', target: 10 }, reward: { exp: 200, equipment: 'leather_armor' }, prereq: 'xc_04' },
  xc_09: { id: 'xc_09', name: '深入竹林', type: 'main', description: '到达竹林深处', requirement: { type: 'explore', target: '竹林' }, reward: { exp: 300 }, prereq: 'xc_08' },
  xc_10: { id: 'xc_10', name: '神秘石碑', type: 'main', description: '调查竹林深处的石碑', requirement: { type: 'level', target: 8 }, reward: { exp: 200 }, prereq: 'xc_09' },
  xc_11: { id: 'xc_11', name: '竹林霸主', type: 'main', description: '击杀竹林BOSS', requirement: { type: 'kill_any', target: 10 }, reward: { exp: 500, equipment: 'bamboo_spear' }, prereq: 'xc_10' },
  xc_12: { id: 'xc_12', name: '离村之路', type: 'main', description: '达到筑基境，离开新手村', requirement: { type: 'level', target: 15 }, reward: { exp: 500, silver: 200 }, prereq: 'xc_11' },
  // ===== 【新增】中原主线任务 =====
  zy_01: { id: 'zy_01', name: '洛阳初探', type: 'main', description: '探索洛阳城', requirement: { type: 'explore', target: '洛阳城' }, reward: { exp: 300, silver: 100 }, prereq: 'xc_12' },
  zy_02: { id: 'zy_02', name: '酒楼听书', type: 'main', description: '在酒楼听传闻', requirement: { type: 'talk', target: '酒楼老板' }, reward: { exp: 200 }, prereq: 'zy_01' },
  zy_03: { id: 'zy_03', name: '黑市初探', type: 'main', description: '到达黑市', requirement: { type: 'explore', target: '黑市' }, reward: { exp: 400 }, prereq: 'zy_01' },
  zy_04: { id: 'zy_04', name: '华山脚下', type: 'main', description: '到达华山脚下', requirement: { type: 'explore', target: '华山脚下' }, reward: { exp: 300 }, prereq: 'zy_01' },
  zy_05: { id: 'zy_05', name: '嵩山猎猪', type: 'main', description: '击杀5只野猪', requirement: { type: 'kill_any', target: 5 }, reward: { exp: 400, silver: 150 }, prereq: 'zy_04' },
  zy_06: { id: 'zy_06', name: '野外探险', type: 'main', description: '到达野外树林', requirement: { type: 'explore', target: '野外树林' }, reward: { exp: 500 }, prereq: 'zy_04' },
  zy_07: { id: 'zy_07', name: '黑熊猎杀', type: 'main', description: '击杀3只黑熊', requirement: { type: 'kill_any', target: 3 }, reward: { exp: 600, silver: 200 }, prereq: 'zy_06' },
  zy_08: { id: 'zy_08', name: '门派消息', type: 'main', description: '听闻门派招收弟子', requirement: { type: 'level', target: 20 }, reward: { exp: 300 }, prereq: 'zy_07' },
  zy_09: { id: 'zy_09', name: '初入山门', type: 'main', description: '到达门派山门', requirement: { type: 'explore', target: '少林寺' }, reward: { exp: 800, silver: 300 }, prereq: 'zy_08' },
  // ===== 【新增】门派主线任务 =====
  mp_01: { id: 'mp_01', name: '入门考验', type: 'main', description: '通过门派入门试炼', requirement: { type: 'level', target: 25 }, reward: { exp: 800 }, prereq: 'zy_09' },
  mp_02: { id: 'mp_02', name: '藏书阁探索', type: 'main', description: '阅读5本典籍', requirement: { type: 'level', target: 28 }, reward: { exp: 800 }, prereq: 'mp_01' },
  mp_03: { id: 'mp_03', name: '门派修炼', type: 'main', description: '修炼功法10次', requirement: { type: 'level', target: 30 }, reward: { exp: 600 }, prereq: 'mp_01' },
  mp_04: { id: 'mp_04', name: '密室探索', type: 'main', description: '发现古老密室', requirement: { type: 'level', target: 35 }, reward: { exp: 1000 }, prereq: 'mp_02' },
  mp_05: { id: 'mp_05', name: '手表激活', type: 'main', description: '获得管理员手表', requirement: { type: 'level', target: 35 }, reward: { exp: 500 }, prereq: 'mp_04' },
  // ===== 【新增】秘境任务 =====
  mj_01: { id: 'mj_01', name: '秘境入口', type: 'main', description: '到达修仙秘境', requirement: { type: 'level', target: 40 }, reward: { exp: 2000 }, prereq: 'mp_05' },
  mj_02: { id: 'mj_02', name: '灵脉探索', type: 'main', description: '探索灵脉', requirement: { type: 'level', target: 42 }, reward: { exp: 1500 }, prereq: 'mj_01' },
  mj_03: { id: 'mj_03', name: '幻境试炼', type: 'main', description: '击败幻影', requirement: { type: 'kill_any', target: 5 }, reward: { exp: 2500 }, prereq: 'mj_02' },
  mj_04: { id: 'mj_04', name: '代码墙', type: 'main', description: '发现代码墙', requirement: { type: 'level', target: 45 }, reward: { exp: 5000 }, prereq: 'mj_03' },
  // ===== 【新增】遗迹任务 =====
  yj_01: { id: 'yj_01', name: '遗迹入口', type: 'main', description: '到达上古遗迹', requirement: { type: 'level', target: 50 }, reward: { exp: 5000 }, prereq: 'mj_04' },
  yj_02: { id: 'yj_02', name: '远古战场', type: 'main', description: '探索远古战场', requirement: { type: 'level', target: 52 }, reward: { exp: 4000 }, prereq: 'yj_01' },
  yj_03: { id: 'yj_03', name: '创世者日记', type: 'main', description: '找到创世者日记', requirement: { type: 'level', target: 55 }, reward: { exp: 8000 }, prereq: 'yj_02' },
  // ===== 【新增】渡劫任务 =====
  dj_01: { id: 'dj_01', name: '天劫降临', type: 'main', description: '到达渡劫仙域', requirement: { type: 'level', target: 60 }, reward: { exp: 10000 }, prereq: 'yj_03' },
  dj_02: { id: 'dj_02', name: '渡劫成功', type: 'main', description: '通过天劫考验', requirement: { type: 'level', target: 65 }, reward: { exp: 15000 }, prereq: 'dj_01' },
  // ===== 【新增】代码空间任务 =====
  dm_01: { id: 'dm_01', name: '代码空间', type: 'main', description: '进入代码空间', requirement: { type: 'level', target: 70 }, reward: { exp: 15000 }, prereq: 'dj_02' },
  dm_02: { id: 'dm_02', name: '源码之心', type: 'main', description: '到达源码之心', requirement: { type: 'level', target: 75 }, reward: { exp: 20000 }, prereq: 'dm_01' },
  dm_03: { id: 'dm_03', name: '最终选择', type: 'main', description: '做出最终选择', requirement: { type: 'level', target: 80 }, reward: { exp: 50000 }, prereq: 'dm_02' },
  // ===== 【新增】支线任务 =====
  xc_s01: { id: 'xc_s01', name: '采药老人', type: 'side', description: '帮药铺老板采集10株灵草', requirement: { type: 'gather', target: 10 }, reward: { exp: 200, silver: 100 }, prereq: null },
  xc_s02: { id: 'xc_s02', name: '猎户的请求', type: 'side', description: '帮猎户击杀10只野兔', requirement: { type: 'kill_any', target: 10 }, reward: { exp: 250, equipment: 'hunters_bow' }, prereq: null },
  xc_s03: { id: 'xc_s03', name: '村庄巡逻', type: 'side', description: '巡逻村庄击退入侵者', requirement: { type: 'kill_any', target: 3 }, reward: { exp: 250, silver: 50 }, prereq: 'xc_08' },
  zy_s01: { id: 'zy_s01', name: '山贼清剿', type: 'side', description: '清除华山脚下的山贼', requirement: { type: 'kill_any', target: 10 }, reward: { exp: 800, silver: 300 }, prereq: 'zy_04' },
  zy_s02: { id: 'zy_s02', name: '黑市交易', type: 'side', description: '在黑市完成3次交易', requirement: { type: 'level', target: 20 }, reward: { exp: 500 }, prereq: 'zy_03' },
  zy_s03: { id: 'zy_s03', name: '毒蛇之患', type: 'side', description: '清除野外树林的毒蛇', requirement: { type: 'kill_any', target: 10 }, reward: { exp: 700 }, prereq: 'zy_06' },
  ...
  // ===== 【补全】新手村隐藏任务 =====
  xc_h01: { id: 'xc_h01', name: '闪烁的秘密', type: 'hidden', description: '调查空气中的闪烁文字', requirement: { type: 'level', target: 5 }, reward: { exp: 500 }, prereq: null },
  xc_h02: { id: 'xc_h02', name: '石碑的真相', type: 'hidden', description: '解读石碑上的二进制', requirement: { type: 'level', target: 8 }, reward: { exp: 300 }, prereq: 'xc_10' },
  xc_h03: { id: 'xc_h03', name: '村长的秘密', type: 'hidden', description: '发现村长知道真相', requirement: { type: 'level', target: 10 }, reward: { exp: 400 }, prereq: 'xc_01' },
  xc_h04: { id: 'xc_h04', name: 'NPC的重复', type: 'hidden', description: '记录NPC重复对话', requirement: { type: 'level', target: 3 }, reward: { exp: 200 }, prereq: null },
  xc_h05: { id: 'xc_h05', name: '空气墙', type: 'hidden', description: '探索村庄边界', requirement: { type: 'level', target: 4 }, reward: { exp: 150 }, prereq: null },
  // ===== 【补全】中原隐藏任务 =====
  zy_h01: { id: 'zy_h01', name: '老玩家的秘密', type: 'hidden', description: '了解老玩家的来历', requirement: { type: 'level', target: 18 }, reward: { exp: 800 }, prereq: 'zy_01' },
  zy_h02: { id: 'zy_h02', name: '黑市商人的真面目', type: 'hidden', description: '发现黑市商人能感知代码', requirement: { type: 'level', target: 20 }, reward: { exp: 1000 }, prereq: 'zy_03' },
  zy_h03: { id: 'zy_h03', name: 'NPC的暂停', type: 'hidden', description: '记录NPC暂停现象', requirement: { type: 'level', target: 15 }, reward: { exp: 500 }, prereq: null },
  zy_h04: { id: 'zy_h04', name: '代码墙', type: 'hidden', description: '发现长安街的代码闪烁', requirement: { type: 'level', target: 16 }, reward: { exp: 600 }, prereq: 'zy_01' },
  zy_h05: { id: 'zy_h05', name: '洛阳城的边界', type: 'hidden', description: '探索洛阳城的空气墙', requirement: { type: 'level', target: 14 }, reward: { exp: 400 }, prereq: 'zy_01' },
  zy_h06: { id: 'zy_h06', name: '酒楼老板的秘密', type: 'hidden', description: '发现酒楼老板知道真相', requirement: { type: 'level', target: 20 }, reward: { exp: 700 }, prereq: 'zy_02' },
  zy_h07: { id: 'zy_h07', name: '传送的山贼', type: 'hidden', description: '记录山贼传送现象', requirement: { type: 'level', target: 15 }, reward: { exp: 500 }, prereq: 'zy_04' },
  zy_h08: { id: 'zy_h08', name: '洛阳城的传说', type: 'hidden', description: '听闻洛阳城的古老传说', requirement: { type: 'level', target: 18 }, reward: { exp: 600 }, prereq: 'zy_01' },
  // ===== 【补全】门派隐藏任务 =====
  mp_h01: { id: 'mp_h01', name: '掌门的秘密', type: 'hidden', description: '发现掌门知道创世者', requirement: { type: 'level', target: 30 }, reward: { exp: 2000 }, prereq: 'mp_01' },
  mp_h02: { id: 'mp_h02', name: 'NPC版本号', type: 'hidden', description: '发现NPC背后的衣服编号', requirement: { type: 'level', target: 28 }, reward: { exp: 1000 }, prereq: 'mp_01' },
  mp_h03: { id: 'mp_h03', name: '藏书阁密室', type: 'hidden', description: '发现藏书阁的隐藏房间', requirement: { type: 'level', target: 32 }, reward: { exp: 1500 }, prereq: 'mp_02' },
  mp_h04: { id: 'mp_h04', name: '正派的真相', type: 'hidden', description: '发现正邪NPC的AI相同', requirement: { type: 'level', target: 35 }, reward: { exp: 1200 }, prereq: 'mp_01' },
  mp_h05: { id: 'mp_h05', name: '门派创始人', type: 'hidden', description: '发现门派创始人是创世者分身', requirement: { type: 'level', target: 35 }, reward: { exp: 2000 }, prereq: 'mp_02' },
  mp_h06: { id: 'mp_h06', name: '手表的来源', type: 'hidden', description: '了解手表的真正来历', requirement: { type: 'level', target: 35 }, reward: { exp: 1500 }, prereq: 'mp_05' },
  mp_h07: { id: 'mp_h07', name: '前穿越者的痕迹', type: 'hidden', description: '发现前穿越者的遗物', requirement: { type: 'level', target: 38 }, reward: { exp: 2000 }, prereq: 'mp_05' },
  // ===== 【补全】秘境隐藏任务 =====
  mj_h01: { id: 'mj_h01', name: '代码墙的秘密', type: 'hidden', description: '深入研究代码墙', requirement: { type: 'level', target: 45 }, reward: { exp: 3000 }, prereq: 'mj_04' },
  mj_h02: { id: 'mj_h02', name: '幻境真相', type: 'hidden', description: '发现幻境是代码构建的', requirement: { type: 'level', target: 43 }, reward: { exp: 2500 }, prereq: 'mj_03' },
  mj_h03: { id: 'mj_h03', name: '灵脉的来源', type: 'hidden', description: '发现灵脉是世界核心的能量', requirement: { type: 'level', target: 42 }, reward: { exp: 2000 }, prereq: 'mj_02' },
  mj_h04: { id: 'mj_h04', name: '秘境NPC', type: 'hidden', description: '发现秘境中的隐藏NPC', requirement: { type: 'level', target: 44 }, reward: { exp: 2500 }, prereq: 'mj_01' },
  mj_h05: { id: 'mj_h05', name: '前穿越者的遗物', type: 'hidden', description: '发现前穿越者在秘境的遗物', requirement: { type: 'level', target: 46 }, reward: { exp: 3000 }, prereq: 'mj_04' },
  mj_h06: { id: 'mj_h06', name: '秘境的代码', type: 'hidden', description: '发现秘境的底层代码', requirement: { type: 'level', target: 45 }, reward: { exp: 2500 }, prereq: 'mj_04' },
  mj_h07: { id: 'mj_h07', name: '创世者的实验', type: 'hidden', description: '发现秘境是创世者的实验场', requirement: { type: 'level', target: 47 }, reward: { exp: 3000 }, prereq: 'mj_04' },
  // ===== 【补全】遗迹隐藏任务 =====
  yj_h01: { id: 'yj_h01', name: '创世者的日记', type: 'hidden', description: '收集所有103篇日记', requirement: { type: 'level', target: 55 }, reward: { exp: 10000 }, prereq: 'yj_03' },
  yj_h02: { id: 'yj_h02', name: '远古大战的真相', type: 'hidden', description: '发现远古大战是代码冲突', requirement: { type: 'level', target: 53 }, reward: { exp: 5000 }, prereq: 'yj_02' },
  yj_h03: { id: 'yj_h03', name: '仙人的身份', type: 'hidden', description: '发现仙人是创世者的分身', requirement: { type: 'level', target: 54 }, reward: { exp: 6000 }, prereq: 'yj_02' },
  yj_h04: { id: 'yj_h04', name: '神兽的来源', type: 'hidden', description: '发现神兽是守护程序', requirement: { type: 'level', target: 55 }, reward: { exp: 4000 }, prereq: 'yj_02' },
  yj_h05: { id: 'yj_h05', name: '遗迹的核心', type: 'hidden', description: '发现遗迹的核心代码', requirement: { type: 'level', target: 56 }, reward: { exp: 8000 }, prereq: 'yj_03' },
  yj_h06: { id: 'yj_h06', name: '前穿越者的失败', type: 'hidden', description: '了解前穿越者失败的原因', requirement: { type: 'level', target: 55 }, reward: { exp: 6000 }, prereq: 'yj_03' },
  yj_h07: { id: 'yj_h07', name: '创世者的意识', type: 'hidden', description: '发现创世者的意识还在运转', requirement: { type: 'level', target: 58 }, reward: { exp: 10000 }, prereq: 'yj_03' },
  yj_h08: { id: 'yj_h08', name: '遗迹的终极秘密', type: 'hidden', description: '发现遗迹是世界核心的入口', requirement: { type: 'level', target: 57 }, reward: { exp: 8000 }, prereq: 'yj_03' },
  // ===== 【补全】渡劫隐藏任务 =====
  dj_h01: { id: 'dj_h01', name: '天劫的本质', type: 'hidden', description: '发现天劫是系统检查', requirement: { type: 'level', target: 63 }, reward: { exp: 12000 }, prereq: 'dj_01' },
  dj_h02: { id: 'dj_h02', name: '飞升通道的代码', type: 'hidden', description: '发现飞升通道的底层代码', requirement: { type: 'level', target: 64 }, reward: { exp: 10000 }, prereq: 'dj_02' },
  dj_h03: { id: 'dj_h03', name: '仙界边缘的真相', type: 'hidden', description: '发现仙界是世界边界', requirement: { type: 'level', target: 65 }, reward: { exp: 8000 }, prereq: 'dj_02' },
  dj_h04: { id: 'dj_h04', name: '源码守护者的秘密', type: 'hidden', description: '发现守护者是创世者的AI', requirement: { type: 'level', target: 65 }, reward: { exp: 15000 }, prereq: 'dj_02' },
  dj_h05: { id: 'dj_h05', name: '前穿越者的日记', type: 'hidden', description: '发现前穿越者的日记', requirement: { type: 'level', target: 64 }, reward: { exp: 12000 }, prereq: 'dj_02' },
  dj_h06: { id: 'dj_h06', name: '创世者的最后留言', type: 'hidden', description: '发现创世者的最后留言', requirement: { type: 'level', target: 66 }, reward: { exp: 20000 }, prereq: 'dj_02' },
  dj_h07: { id: 'dj_h07', name: '世界的真相', type: 'hidden', description: '发现世界的全部真相', requirement: { type: 'level', target: 67 }, reward: { exp: 15000 }, prereq: 'dj_02' },
  dj_h08: { id: 'dj_h08', name: '代码空间的钥匙', type: 'hidden', description: '获得代码空间的钥匙', requirement: { type: 'level', target: 68 }, reward: { exp: 10000 }, prereq: 'dj_02' },
  // ===== 【补全】代码空间隐藏任务 =====
  dm_h01: { id: 'dm_h01', name: 'Bug的真相', type: 'hidden', description: '发现Bug是创世者意识衰弱', requirement: { type: 'level', target: 72 }, reward: { exp: 12000 }, prereq: 'dm_01' },
  dm_h02: { id: 'dm_h02', name: '遗物的真相', type: 'hidden', description: '发现遗物是创世者的记忆', requirement: { type: 'level', target: 73 }, reward: { exp: 15000 }, prereq: 'dm_01' },
  dm_h03: { id: 'dm_h03', name: '源码的真相', type: 'hidden', description: '发现源码是世界的核心', requirement: { type: 'level', target: 75 }, reward: { exp: 20000 }, prereq: 'dm_02' },
  dm_h04: { id: 'dm_h04', name: '创世者的真相', type: 'hidden', description: '发现创世者的全部秘密', requirement: { type: 'level', target: 78 }, reward: { exp: 25000 }, prereq: 'dm_02' },
  dm_h05: { id: 'dm_h05', name: '世界的真相', type: 'hidden', description: '发现世界的全部真相', requirement: { type: 'level', target: 80 }, reward: { exp: 20000 }, prereq: 'dm_02' },
  dm_h06: { id: 'dm_h06', name: '代码的永恒', type: 'hidden', description: '理解代码的永恒本质', requirement: { type: 'level', target: 80 }, reward: { exp: 15000 }, prereq: 'dm_02' },
  dm_h07: { id: 'dm_h07', name: '继承者的选择', type: 'hidden', description: '选择成为新的创世者', requirement: { type: 'level', target: 80 }, reward: { exp: 30000 }, prereq: 'dm_03' },
  dm_h08: { id: 'dm_h08', name: '回归的选择', type: 'hidden', description: '选择回到现实世界', requirement: { type: 'level', target: 80 }, reward: { exp: 30000 }, prereq: 'dm_03' },
  dm_h09: { id: 'dm_h09', name: '代码的最终秘密', type: 'hidden', description: '发现代码的最终秘密', requirement: { type: 'level', target: 80 }, reward: { exp: 50000 }, prereq: 'dm_03' },
  // ===== 【补全】新地图任务 =====
  jn_01: { id: 'jn_01', name: '水乡初探', type: 'main', description: '到达江南水乡', requirement: { type: 'level', target: 18 }, reward: { exp: 600 }, prereq: 'xc_12' },
  jn_02: { id: 'jn_02', name: '渔村帮忙', type: 'side', description: '帮渔民捕鱼', requirement: { type: 'kill_any', target: 5 }, reward: { exp: 400, silver: 150 }, prereq: 'jn_01' },
  jn_03: { id: 'jn_03', name: '药田采集', type: 'side', description: '在药田采集灵草', requirement: { type: 'gather', target: 10 }, reward: { exp: 500, silver: 200 }, prereq: 'jn_01' },
  jn_04: { id: 'jn_04', name: '湖妖猎杀', type: 'side', description: '击杀湖妖', requirement: { type: 'kill_any', target: 3 }, reward: { exp: 800, silver: 300 }, prereq: 'jn_01' },
  sm_01: { id: 'sm_01', name: '沙漠探险', type: 'main', description: '到达沙漠戈壁', requirement: { type: 'level', target: 30 }, reward: { exp: 1500 }, prereq: 'mp_01' },
  sm_02: { id: 'sm_02', name: '绿洲生存', type: 'side', description: '在绿洲击败怪物', requirement: { type: 'kill_any', target: 5 }, reward: { exp: 1200, silver: 400 }, prereq: 'sm_01' },
  sm_03: { id: 'sm_03', name: '古城探索', type: 'side', description: '探索古城遗迹', requirement: { type: 'level', target: 32 }, reward: { exp: 1500 }, prereq: 'sm_01' },
  sm_04: { id: 'sm_04', name: '密室解谜', type: 'side', description: '解开地下密室谜题', requirement: { type: 'level', target: 35 }, reward: { exp: 2000 }, prereq: 'sm_03' },
  bj_01: { id: 'bj_01', name: '冰原探索', type: 'main', description: '到达北境冰原', requirement: { type: 'level', target: 40 }, reward: { exp: 2000 }, prereq: 'mj_01' },
  bj_02: { id: 'bj_02', name: '冰狼猎杀', type: 'side', description: '击杀冰狼', requirement: { type: 'kill_any', target: 5 }, reward: { exp: 1800, silver: 600 }, prereq: 'bj_01' },
  bj_03: { id: 'bj_03', name: '冰龙挑战', type: 'side', description: '挑战冰龙', requirement: { type: 'kill_any', target: 1 }, reward: { exp: 3000, silver: 1000 }, prereq: 'bj_01' },
  bj_04: { id: 'bj_04', name: '雪山之巅', type: 'side', description: '到达雪山之巅', requirement: { type: 'level', target: 42 }, reward: { exp: 2500 }, prereq: 'bj_01' },
  nj_01: { id: 'nj_01', name: '密林探索', type: 'main', description: '到达南疆密林', requirement: { type: 'level', target: 40 }, reward: { exp: 2000 }, prereq: 'mj_01' },
  nj_02: { id: 'nj_02', name: '毒蛙猎杀', type: 'side', description: '击杀毒蛙', requirement: { type: 'kill_any', target: 5 }, reward: { exp: 1500, silver: 500 }, prereq: 'nj_01' },
  nj_03: { id: 'nj_03', name: '蛊王挑战', type: 'side', description: '挑战蛊王', requirement: { type: 'kill_any', target: 1 }, reward: { exp: 2500, silver: 800 }, prereq: 'nj_01' },
  nj_04: { id: 'nj_04', name: '巫蛊祭坛', type: 'side', description: '到达巫蛊祭坛', requirement: { type: 'level', target: 42 }, reward: { exp: 2000 }, prereq: 'nj_01' },
  ht_01: { id: 'ht_01', name: '海底探索', type: 'main', description: '到达海底龙宫', requirement: { type: 'level', target: 50 }, reward: { exp: 5000 }, prereq: 'yj_01' },
  ht_02: { id: 'ht_02', name: '深海猎杀', type: 'side', description: '击杀深海巨鲸', requirement: { type: 'kill_any', target: 1 }, reward: { exp: 4000, silver: 1500 }, prereq: 'ht_01' },
  ht_03: { id: 'ht_03', name: '龙宫探索', type: 'side', description: '探索龙宫大殿', requirement: { type: 'level', target: 52 }, reward: { exp: 5000 }, prereq: 'ht_01' },
  ht_04: { id: 'ht_04', name: '龙王挑战', type: 'side', description: '挑战龙王', requirement: { type: 'kill_any', target: 1 }, reward: { exp: 8000, silver: 3000 }, prereq: 'ht_01' },
  xk_01: { id: 'xk_01', name: '虚空探索', type: 'main', description: '到达虚空裂缝', requirement: { type: 'level', target: 60 }, reward: { exp: 10000 }, prereq: 'dj_01' },
  xk_02: { id: 'xk_02', name: '时空走廊', type: 'side', description: '穿越时空走廊', requirement: { type: 'level', target: 62 }, reward: { exp: 8000 }, prereq: 'xk_01' },
  xk_03: { id: 'xk_03', name: '虚无之海', type: 'side', description: '探索虚无之海', requirement: { type: 'level', target: 64 }, reward: { exp: 10000 }, prereq: 'xk_01' },
  xk_04: { id: 'xk_04', name: '虚空帝王', type: 'side', description: '挑战虚空帝王', requirement: { type: 'kill_any', target: 1 }, reward: { exp: 15000, silver: 5000 }, prereq: 'xk_01' }
};
...
const RELICS_DATA = {
  // Category 1: 十二月遗物 (Monthly Relics) - 12件
  M01: { id: 'M01', name: '正月·春雷碎片', category: '十二月', type: 'month', desc: '悟性+5%, 突破率+3%', effects: { wisdom: 0.05, breakthrough: 0.03 }, puzzleWeight: 1 },
  M02: { id: 'M02', name: '二月·惊蛰之种', category: '十二月', type: 'month', desc: 'HP+8%, 回复+15%', effects: { hp: 0.08, regen: 0.15 }, puzzleWeight: 2 },
  M03: { id: 'M03', name: '三月·清明露珠', category: '十二月', type: 'month', desc: '灵力+10%, 炼丹+5%', effects: { spirit: 0.10, alchemy: 0.05 }, puzzleWeight: 3 },
  M04: { id: 'M04', name: '四月·谷雨精华', category: '十二月', type: 'month', desc: '经验+5%, 体力恢复+10%', effects: { exp: 0.05, staminaRecovery: 0.10 }, puzzleWeight: 4 },
  M05: { id: 'M05', name: '五月·端午龙鳞', category: '十二月', type: 'month', desc: '防御+8%, 毒抗+20%', effects: { defense: 0.08, poisonResist: 0.20 }, puzzleWeight: 5 },
  M06: { id: 'M06', name: '六月·夏至烈焰', category: '十二月', type: 'month', desc: '攻击+10%, 暴击伤害+15%', effects: { attack: 0.10, critDamage: 0.15 }, puzzleWeight: 6 },
  M07: { id: 'M07', name: '七月·小暑流火', category: '十二月', type: 'month', desc: '攻击+12%, 火焰伤害+10%', effects: { attack: 0.12 }, puzzleWeight: 7 },
  M08: { id: 'M08', name: '八月·立秋霜华', category: '十二月', type: 'month', desc: '冰抗+15%, 速度+8%', effects: { speed: 0.08, defense: 0.06 }, puzzleWeight: 8 },
  M09: { id: 'M09', name: '九月·白露凝珠', category: '十二月', type: 'month', desc: '回复+20%, 生命上限+5%', effects: { regen: 0.20, hp: 0.05 }, puzzleWeight: 9 },
  M10: { id: 'M10', name: '十月·寒露玄冰', category: '十二月', type: 'month', desc: '防御+12%, 冰抗+10%', effects: { defense: 0.12 }, puzzleWeight: 10 },
  M11: { id: 'M11', name: '十一月·大雪封山', category: '十二月', type: 'month', desc: '体力上限+15%, 闪避+8%', effects: { dodge: 0.08, staminaRecovery: 0.15 }, puzzleWeight: 11 },
  M12: { id: 'M12', name: '十二月·冬至归元', category: '十二月', type: 'month', desc: '全属性+5%, 悟性+8%', effects: { wisdom: 0.08, spirit: 0.05 }, puzzleWeight: 12 },
  // Category 2: 十二时辰遗物 (Time Relics) - 12件
  T01: { id: 'T01', name: '子时·夜半钟声', category: '十二时辰', type: 'hour', desc: '挂机经验+8%, 夜间修炼+15%', effects: { idleExp: 0.08, nightCultivation: 0.15 }, puzzleWeight: 1 },
  T02: { id: 'T02', name: '丑时·鸡鸣破晓', category: '十二时辰', type: 'hour', desc: '闪避+10%, 先手+8%', effects: { dodge: 0.10, initiative: 0.08 }, puzzleWeight: 2 },
  T03: { id: 'T03', name: '寅时·平旦微光', category: '十二时辰', type: 'hour', desc: 'HP回复+12%, 减益时间-20%', effects: { hpRegen: 0.12, debuffReduction: 0.20 }, puzzleWeight: 3 },
  T04: { id: 'T04', name: '卯时·日出东方', category: '十二时辰', type: 'hour', desc: '经验+8%, 悟性+10%', effects: { exp: 0.08, wisdom: 0.10 }, puzzleWeight: 4 },
  T05: { id: 'T05', name: '辰时·食时之宴', category: '十二时辰', type: 'hour', desc: '体力恢复+15%, 食物效果+20%', effects: { staminaRecovery: 0.15 }, puzzleWeight: 5 },
  T06: { id: 'T06', name: '巳时·隅中之风', category: '十二时辰', type: 'hour', desc: '速度+12%, 闪避+8%', effects: { speed: 0.12, dodge: 0.08 }, puzzleWeight: 6 },
  T07: { id: 'T07', name: '午时·日中之火', category: '十二时辰', type: 'hour', desc: '攻击+15%, 暴击率+8%', effects: { attack: 0.15, critRate: 0.08 }, puzzleWeight: 7 },
  T08: { id: 'T08', name: '未时·日昳之影', category: '十二时辰', type: 'hour', desc: '隐匿+15%, 偷袭+10%', effects: { dodge: 0.15 }, puzzleWeight: 8 },
  T09: { id: 'T09', name: '申时·晡时之金', category: '十二时辰', type: 'hour', desc: '银两+15%, 掉落+10%', effects: { lootBonus: 0.15 }, puzzleWeight: 9 },
  T10: { id: 'T10', name: '酉时·日入之霞', category: '十二时辰', type: 'hour', desc: '灵力+12%, 炼丹+10%', effects: { spirit: 0.12, alchemy: 0.10 }, puzzleWeight: 10 },
  T11: { id: 'T11', name: '戌时·黄昏之灯', category: '十二时辰', type: 'hour', desc: '防御+10%, 生命回复+15%', effects: { defense: 0.10, hpRegen: 0.15 }, puzzleWeight: 11 },
  T12: { id: 'T12', name: '亥时·人定之梦', category: '十二时辰', type: 'hour', desc: '挂机经验+12%, 道心+5%', effects: { idleExp: 0.12 }, puzzleWeight: 12 },
  // Category 3: 十二生肖遗物 (Zodiac Relics) - 12件
  Z01: { id: 'Z01', name: '子鼠·窃运之爪', category: '十二生肖', type: 'zodiac', desc: '偷取+10%, 掉落+5%', effects: { stealChance: 0.10, lootBonus: 0.05 }, puzzleWeight: 1 },
  Z02: { id: 'Z02', name: '丑牛·蛮力之心', category: '十二生肖', type: 'zodiac', desc: '力量+15%, 负重+30%', effects: { strength: 0.15, carryWeight: 0.30 }, puzzleWeight: 2 },
  Z03: { id: 'Z03', name: '寅虎·啸风之牙', category: '十二生肖', type: 'zodiac', desc: '暴击率+10%, 恐惧+8%', effects: { critRate: 0.10, fearChance: 0.08 }, puzzleWeight: 3 },
  Z04: { id: 'Z04', name: '卯兔·月华之耳', category: '十二生肖', type: 'zodiac', desc: '闪避+12%, 速度+10%', effects: { dodge: 0.12, speed: 0.10 }, puzzleWeight: 4 },
  Z05: { id: 'Z05', name: '辰龙·龙吟之鳞', category: '十二生肖', type: 'zodiac', desc: '全属性+8%, 威压+10%', effects: { attack: 0.08, defense: 0.08, hp: 0.08, spirit: 0.08 }, puzzleWeight: 5 },
  Z06: { id: 'Z06', name: '巳蛇·蛇信之毒', category: '十二生肖', type: 'zodiac', desc: '毒伤害+20%, 毒抗+15%', effects: { poisonResist: 0.20 }, puzzleWeight: 6 },
  Z07: { id: 'Z07', name: '午马·千里之蹄', category: '十二生肖', type: 'zodiac', desc: '速度+15%, 移动加速', effects: { speed: 0.15 }, puzzleWeight: 7 },
  Z08: { id: 'Z08', name: '未羊·祥瑞之角', category: '十二生肖', type: 'zodiac', desc: '奇遇+15%, 道心+8%', effects: { encounterBonus: 0.15 }, puzzleWeight: 8 },
  Z09: { id: 'Z09', name: '申猴·灵巧之手', category: '十二生肖', type: 'zodiac', desc: '炼丹+15%, 锻造+10%', effects: { alchemy: 0.15 }, puzzleWeight: 9 },
  Z10: { id: 'Z10', name: '酉鸡·晨鸣之冠', category: '十二生肖', type: 'zodiac', desc: '经验+10%, 悟性+12%', effects: { exp: 0.10, wisdom: 0.12 }, puzzleWeight: 10 },
  Z11: { id: 'Z11', name: '戌狗·忠诚之心', category: '十二生肖', type: 'zodiac', desc: '防御+12%, 友军加成+10%', effects: { defense: 0.12 }, puzzleWeight: 11 },
  Z12: { id: 'Z12', name: '亥猪·福运之鼻', category: '十二生肖', type: 'zodiac', desc: '银两+20%, 掉率+12%', effects: { lootBonus: 0.20 }, puzzleWeight: 12 }
};

// ===== RELIC SET BONUS SYSTEM (遗物组合效果) =====
// Set bonus: 3件+5%, 6件+15%, 9件+30%, 12件解锁专属技能
const RELIC_SET_SKILLS = {
  month: { name: '岁月长河', desc: '十二月遗物集齐：全属性+50%，每回合回复10%HP' },
  hour: { name: '时间之轮', desc: '十二时辰遗物集齐：经验+50%，挂机经验翻倍' },
  zodiac: { name: '生肖轮回', desc: '十二生肖遗物集齐：暴击+50%，掉落翻倍' }
};

function getRelicSetBonus(player) {
  const types = { month: 0, hour: 0, zodiac: 0 };
  const relics = player.relics || [];
  for (const relicId of relics) {
    const r = RELICS_DATA[relicId];
    if (r && r.type && types[r.type] !== undefined) types[r.type]++;
  }
  let bonus = {};
  for (const [type, count] of Object.entries(types)) {
    if (count >= 12) bonus[type] = { pct: 50, skill: true };
    else if (count >= 9) bonus[type] = { pct: 30 };
    else if (count >= 6) bonus[type] = { pct: 15 };
    else if (count >= 3) bonus[type] = { pct: 5 };
  }
  return bonus;
}

// Admin Watch level thresholds (Lv0-Lv9, exp-based)
const WATCH_EXP = [0, 100, 300, 600, 1000, 1500, 2100, 2800, 3600, 4500];
const WATCH_THRESHOLDS = [
  { level: 0, exp: 0, abilities: '基础显示' },
  { level: 1, exp: 100, abilities: '隐藏属性可见(怪物真实HP)' },
  { level: 2, exp: 300, abilities: 'Bug扫描解锁' },
  { level: 3, exp: 600, abilities: '代码片段查看, 自动Bug任务, 瞬移(5次/天)' },
  { level: 4, exp: 1000, abilities: '源码碎片, 战斗预测, 经验+10%' },
  { level: 5, exp: 1500, abilities: '数据流监控, 经验+15%' },
  { level: 6, exp: 2100, abilities: '系统漏洞利用, 掉率+20%' },
  { level: 7, exp: 2800, abilities: '底层代码修改, 全属性+10%' },
  { level: 8, exp: 3600, abilities: '核心权限解锁, 暴击+15%' },
  { level: 9, exp: 4500, abilities: '创世者权限, 全属性+20%, 解锁真结局' }
];

function calcWatchLevel(watchExp) {
  for (let i = WATCH_EXP.length - 1; i >= 0; i--) {
    if (watchExp >= WATCH_EXP[i]) return i;
  }
  return 0;
}

// ===== BUG EXPLOITATION SYSTEM =====
// 32种Bug，分4类：界面(8)、逻辑(8)、数据(8)、系统(8)
const BUGS_DATA = {
  // === 界面Bug (Display Bugs) - 8种 ===
  B01: { id: 'B01', name: '时间膨胀泡', category: '经验', bugType: 'display', desc: '挂机经验x3 持续10分钟', dailyLimit: 3, duration: 600000, fixReq: { gold: 100 } },
  B02: { id: 'B02', name: '经验溢出点', category: '经验', bugType: 'display', desc: '战斗经验x2 持续1小时', dailyLimit: 1, duration: 3600000, fixReq: { gold: 200 } },
  B03: { id: 'B03', name: '显示闪烁', category: '界面', bugType: 'display', desc: '界面闪烁，额外获得50经验', dailyLimit: 5, duration: 0, fixReq: { gold: 50 } },
  B04: { id: 'B04', name: '纹理错位', category: '界面', bugType: 'display', desc: '怪物防御-20% 持续10分钟', dailyLimit: 2, duration: 600000, fixReq: { gold: 80 } },
  B05: { id: 'B05', name: '渲染残影', category: '界面', bugType: 'display', desc: '闪避+15% 持续30分钟', dailyLimit: 2, duration: 1800000, fixReq: { gold: 120 } },
  B06: { id: 'B06', name: '色彩反转', category: '界面', bugType: 'display', desc: '暴击率+10% 持续20分钟', dailyLimit: 2, duration: 1200000, fixReq: { gold: 100 } },
  B07: { id: 'B07', name: '字体溢出', category: '界面', bugType: 'display', desc: '经验+50直接获得', dailyLimit: 3, duration: 0, fixReq: { gold: 60 } },
  B08: { id: 'B08', name: 'UI层叠', category: '界面', bugType: 'display', desc: '查看隐藏怪物信息30分钟', dailyLimit: 1, duration: 1800000, fixReq: { gold: 150 } },
  // === 逻辑Bug (Logic Bugs) - 8种 ===
  B11: { id: 'B11', name: '背包溢出', category: '物品', bugType: 'logic', desc: '临时+100背包格 1小时', dailyLimit: 1, duration: 3600000, fixReq: { gold: 200 } },
  B12: { id: 'B12', name: '商店漏洞', category: '物品', bugType: 'logic', desc: '免费随机商店物品', dailyLimit: 1, duration: 0, fixReq: { gold: 150 } },
  B13: { id: 'B13', name: '数值溢出', category: '逻辑', bugType: 'logic', desc: '下次攻击伤害x5', dailyLimit: 1, duration: 0, fixReq: { gold: 300 } },
  B14: { id: 'B14', name: '循环漏洞', category: '逻辑', bugType: 'logic', desc: '挂机经验x5 持续5分钟', dailyLimit: 1, duration: 300000, fixReq: { gold: 250 } },
  B15: { id: 'B15', name: '递归溢出', category: '逻辑', bugType: 'logic', desc: '连续攻击3次', dailyLimit: 2, duration: 0, fixReq: { gold: 180 } },
  B16: { id: 'B16', name: '条件判断错误', category: '逻辑', bugType: 'logic', desc: '怪物不攻击1回合', dailyLimit: 2, duration: 0, fixReq: { gold: 120 } },
  B18: { id: 'B18', name: '优先级反转', category: '逻辑', bugType: 'logic', desc: '先手攻击+30% 持续30分钟', dailyLimit: 1, duration: 1800000, fixReq: { gold: 160 } },
  B19: { id: 'B19', name: '类型混淆', category: '逻辑', bugType: 'logic', desc: '防御转攻击30% 持续15分钟', dailyLimit: 1, duration: 900000, fixReq: { gold: 200 } },
  // === 数据Bug (Data Bugs) - 8种 ===
  B22: { id: 'B22', name: '存档延迟', category: '数据', bugType: 'data', desc: '签到奖励翻倍(下次签到)', dailyLimit: 1, duration: 0, fixReq: { gold: 100 } },
  B23: { id: 'B23', name: '数据残留', category: '数据', bugType: 'data', desc: '获得上一个击杀怪物的经验', dailyLimit: 3, duration: 0, fixReq: { gold: 80 } },
  B24: { id: 'B24', name: '缓存污染', category: '数据', bugType: 'data', desc: '随机获得1个已知Bug效果', dailyLimit: 2, duration: 0, fixReq: { gold: 120 } },
  B25: { id: 'B25', name: '索引错乱', category: '数据', bugType: 'data', desc: '随机传送已探索房间', dailyLimit: 3, duration: 0, fixReq: { gold: 60 } },
  B26: { id: 'B26', name: '内存泄漏', category: '数据', bugType: 'data', desc: '灵力+50直接获得', dailyLimit: 2, duration: 0, fixReq: { gold: 100 } },
  B27: { id: 'B27', name: '指针偏移', category: '数据', bugType: 'data', desc: '随机属性+1(永久)', dailyLimit: 1, duration: 0, fixReq: { gold: 500 } },
  B28: { id: 'B28', name: '数据重复', category: '数据', bugType: 'data', desc: '下次掉落物品x2', dailyLimit: 1, duration: 0, fixReq: { gold: 200 } },
  B29: { id: 'B29', name: '记录丢失', category: '数据', bugType: 'data', desc: '消除1个负面状态', dailyLimit: 2, duration: 0, fixReq: { gold: 80 } },
  // === 系统Bug (System Bugs) - 8种 ===
  B31: { id: 'B31', name: '伤害溢出', category: '战斗', bugType: 'system', desc: '下次攻击伤害x3', dailyLimit: 3, duration: 0, fixReq: { gold: 100 } },
  B33: { id: 'B33', name: '怪物卡住', category: '战斗', bugType: 'system', desc: '怪物跳过1回合', dailyLimit: 3, duration: 0, fixReq: { gold: 80 } },
  B34: { id: 'B34', name: '规则漏洞', category: '系统', bugType: 'system', desc: '境界突破率+30% 持续1小时', dailyLimit: 1, duration: 3600000, fixReq: { gold: 500 } },
  B35: { id: 'B35', name: '权限提升', category: '系统', bugType: 'system', desc: '临时管理员权限5分钟', dailyLimit: 1, duration: 300000, fixReq: { gold: 400 } },
  B36: { id: 'B36', name: '时间回溯', category: '系统', bugType: 'system', desc: '恢复上次死亡前的HP', dailyLimit: 1, duration: 0, fixReq: { gold: 300 } },
  B37: { id: 'B37', name: '世界线偏移', category: '系统', bugType: 'system', desc: '奇遇触发率x3 持续30分钟', dailyLimit: 1, duration: 1800000, fixReq: { gold: 250 } },
  B45: { id: 'B45', name: '道心漏洞', category: '特殊', bugType: 'system', desc: '道心+200 持续1小时', dailyLimit: 1, duration: 3600000, fixReq: { gold: 300 } },
  B49: { id: 'B49', name: '体力漏洞', category: '特殊', bugType: 'system', desc: '无限体力30分钟', dailyLimit: 1, duration: 1800000, fixReq: { gold: 200 } },
  B50: { id: 'B50', name: '掉率漏洞', category: '特殊', bugType: 'system', desc: '掉率x5 持续1小时', dailyLimit: 1, duration: 3600000, fixReq: { gold: 350 } },
  B17: { id: 'B17', name: '瞬移漏洞', category: '传送', bugType: 'system', desc: '传送到已探索地图', dailyLimit: 3, duration: 0, fixReq: { gold: 100 } },
  B20: { id: 'B20', name: '回城Bug', category: '传送', bugType: 'system', desc: '瞬间回到新手村', dailyLimit: 999, duration: 0, fixReq: { gold: 20 } },
  B21: { id: 'B21', name: '传送门', category: '传送', bugType: 'system', desc: '创建传送门30分钟', dailyLimit: 1, duration: 1800000, fixReq: { gold: 150 } }
};

// Bug fix tracking - all 32 bug IDs for true ending check
const ALL_BUG_IDS = Object.keys(BUGS_DATA);

// Bug discovery locations (expanded for 32 bugs)
const BUG_LOCATIONS = {
  '竹林': ['B01', 'B33', 'B03', 'B15'],
  '村长屋': ['B12', 'B45', 'B08', 'B35'],
  '集市': ['B11', 'B50', 'B04', 'B28'],
  ...
  '铁匠铺': ['B31', 'B13', 'B18', 'B36'],
  '修炼场': ['B17', 'B20', 'B14', 'B34'],
  'Bug走廊': ['B05', 'B16', 'B19', 'B23', 'B24', 'B25', 'B27', 'B37'],
  '源码之心': ['B05', 'B16', 'B19', 'B23', 'B24', 'B25', 'B27', 'B37']
};
...
function handleCommand(ws, data) {
  ...
  // ===== Phase 6: Cyber Core Extensions =====
  } else if (command.startsWith('/VIP')) {
    handleVipCommand(ws, player, command);
  } else if (command.startsWith('/生成卡密')) {
    handleGenerateVipKeyCommand(ws, player, command);
  } else if (command.startsWith('/解谜')) {
    handlePuzzleCommand(ws, player);
  } else if (command.startsWith('/谜题提示')) {
    handlePuzzleHintCommand(ws, player);
  } else if (command.startsWith('/修复Bug')) {
    const bugId = command.replace('/修复Bug', '').trim();
    handleFixBugCommand(ws, player, bugId);
  } else if (command.startsWith('/结局进度')) {
    handleEndingProgressCommand(ws, player);
  } else if (command.startsWith('/选择结局')) {
    handleChooseEndingCommand(ws, player);
  // ===== Phase 7: 收尾打磨 =====
  } else if (command.startsWith('/成就')) {
    handleAchievementCommand(ws, player);
  } else if (command.startsWith('/排行榜')) {
    handleLeaderboardCommand(ws, player);
  } else if (command.startsWith('/BOSS')) {
    handleBossCommand(ws, player, command);
  } else if (command.startsWith('/主线')) {
    handleMainQuestCommand(ws, player, command);
  } else if (command.startsWith('/对话')) {
    handleNpcDialogCommand(ws, player, command);
  } else if (command.startsWith('/随机事件')) {
    handleRandomEventCommand(ws, player);
  } else {
    sendToClient(ws, {
      type: 'error',
...
function handleSignInCommand(ws, player) {
  ...
  // VIP bonus: +10% silver, extra pill
  if (isVipActive(p)) {
    const vipBonus = Math.floor(silverReward * getVipSilverBonus(p));
    silverReward += vipBonus;
    pillCount = 2;
    signMsg = '\n👑 VIP特权: 灵石+' + vipBonus + ', 额外丹药x1';
  }
  
  p.silver += silverReward;
  // Random pill reward
  const pillIds = Object.keys(PILLS_DATA);
  const randomPillId = pillIds[Math.floor(Math.random() * pillIds.length)];
  if (!p.pills) p.pills = {};
  p.pills[randomPillId] = (p.pills[randomPillId] || 0) + pillCount;
  const pd = PILLS_DATA[randomPillId];
  savePlayers(players);
  sendToClient(ws, {
    type: 'system',
    data: { message: `✅ 签到成功！获得 ${silverReward} 灵石和【${pd.name}】x${pillCount}！${signMsg}` }
  });
}
...
// ===== CYBER SYSTEM HELPER =====
// calcWatchLevel is now defined above (exp-based, Lv0-Lv9)

// ===== VIP SYSTEM CONFIG =====
const VIP_CONFIG = {
  week:  { days: 7,   name: '周卡', bonusExp: 0.2, bonusSilver: 0.1, title: 'VIP·周卡贵宾' },
  month: { days: 30,  name: '月卡', bonusExp: 0.2, bonusSilver: 0.1, title: 'VIP·月卡贵宾' },
  year:  { days: 365, name: '年卡', bonusExp: 0.2, bonusSilver: 0.1, title: 'VIP·年卡至尊' }
};

// VIP card key storage file
const VIP_KEYS_FILE = path.join(DATA_DIR, 'vip_keys.json');

// Generate a VIP key
function generateVipKey(type) {
  ...
  return { code, type, usedBy: null, createdAt: Date.now() };
}

// Get VIP bonus multiplier for exp
function getVipExpBonus(player) {
  if (!player.vipExpiry || Date.now() > player.vipExpiry) return 0;
  const config = VIP_CONFIG[player.vipType];
  return config ? config.bonusExp : 0;
}

// Get VIP bonus multiplier for silver
function getVipSilverBonus(player) {
  if (!player.vipExpiry || Date.now() > player.vipExpiry) return 0;
  const config = VIP_CONFIG[player.vipType];
  return config ? config.bonusSilver : 0;
}

// Check if player has active VIP
function isVipActive(player) {
  return player.vipExpiry && Date.now() < player.vipExpiry;
}
...
// /遗物 - show collected relics (expanded to 36)
function handleRelicCommand(ws, player) {
  const relics = player.relics || [];
  const totalRelics = Object.keys(RELICS_DATA).length; // 36
  if (relics.length === 0) {
    sendToClient(ws, {
      type: 'command_response',
      data: {
        title: '程序遗物',
        content: `\n╔══════════════════════════════════╗\n║  程序遗物 (共${totalRelics}件)\n╠══════════════════════════════════╣\n║  你还没有发现任何程序遗物。\n║\n║  击败怪物时有极小概率掉落。\n║  程序遗物是这个世界的源代码碎片...\n║  作为2030年的退休程序员，\n║  你似乎能感知到它们的存在。\n║\n║  三类遗物:\n║  · 十二月遗物 (12件)\n║  · 十二时辰遗物 (12件)\n║  · 十二生肖遗物 (12件)\n╚══════════════════════════════════╝`
      }
    });
    return;
  }
  ...
  // Show set bonuses
  const setBonus = getRelicSetBonus(player);
  let bonusText = '';
  if (Object.keys(setBonus).length > 0) {
    bonusText += '╠══════════════════════════════════╣\n║  组合效果:\n';
    for (const [type, info] of Object.entries(setBonus)) {
      const typeName = type === 'month' ? '十二月' : type === 'hour' ? '十二时辰' : '十二生肖';
      bonusText += `║    ${typeName}: +${info.pct}%${info.skill ? ' +专属技能' : ''}\n`;
      if (info.skill && RELIC_SET_SKILLS[type]) {
        bonusText += `║    → ${RELIC_SET_SKILLS[type].name}: ${RELIC_SET_SKILLS[type].desc}\n`;
      }
    }
  }
  
  const watchStatus = player.adminWatch ? `Lv${player.adminWatchLevel || 0}` : '未获得';
  
  sendToClient(ws, {
    type: 'command_response',
    data: {
      title: '程序遗物',
      content: `\n╔══════════════════════════════════╗\n║  程序遗物 (已收集 ${relics.length}/${totalRelics})\n╠══════════════════════════════════╣\n${list}${bonusText}╠══════════════════════════════════╣\n║  管理员手表: ${watchStatus}\n║  手表详情: /手表\n║  解谜: /解谜 | 结局: /结局进度\n╚══════════════════════════════════╝`
    }
  });
}

// /手表 - show admin watch status (Lv0-Lv9, exp-based)
function handleWatchCommand(ws, player) {
  ...
  const watchExp = player.watchExp || 0;
  const level = calcWatchLevel(watchExp);
  player.adminWatchLevel = level; // sync
  const relicCount = (player.relics || []).length;
  const nextLevelExp = level < 9 ? WATCH_EXP[level + 1] : WATCH_EXP[9];
  const expProgress = level < 9 ? `${watchExp}/${nextLevelExp}` : '已满级';
  
  let abilities = '';
  for (let i = 0; i <= 9; i++) {
    const wt = WATCH_THRESHOLDS[i];
    const marker = i <= level ? '✅' : '🔒';
    abilities += `║  ${marker} Lv${wt.level} (${wt.exp}经验): ${wt.abilities}\n`;
  }
  ...
  if (isVipActive(player)) {
    const remaining = Math.ceil((player.vipExpiry - Date.now()) / 86400000);
    vipInfo = `║  VIP: ${player.vipTitle || 'VIP'} (${remaining}天)\n`;
  }
    ...
    // ===== RELIC DROP CHECK =====
    {
      ...
      if (Math.random() < relicChance) {
        ...
        if (available.length > 0) {
          ...
          // Update admin watch if player has the watch - give watchExp
          if (player.adminWatch) {
            const oldLevel = player.adminWatchLevel || 0;
            player.watchExp = (player.watchExp || 0) + 50; // +50 watch exp per relic
            const newLevel = calcWatchLevel(player.watchExp);
...
// ===== PHASE 6: CYBER CORE EXTENSION COMMANDS =====

// /VIP <卡密> - 激活VIP
function handleVipCommand(ws, player, command) {
  const key = command.replace('/VIP', '').trim();
  if (!key) {
    // Show VIP status
    if (isVipActive(player)) {
      const remaining = Math.ceil((player.vipExpiry - Date.now()) / 86400000);
      ...
      sendToClient(ws, {
        ...
        data: {
          title: 'VIP状态',
          content: `\n╔══════════════════════════════════╗\n║  VIP状态\n╠══════════════════════════════════╣\n║  类型: ${config ? config.name : player.vipType}\n║  称号: ${player.vipTitle || 'VIP'}\n║  剩余: ${remaining}天\n║  效果: 经验+20%, 灵石+10%\n║  签到: 额外丹药\n╠══════════════════════════════════╣\n║  使用 /VIP <卡密> 续费\n╚══════════════════════════════════╝`
        }
      });
    } else {
      sendToClient(ws, {
  ...
  // Validate key format
  if (!/^VIP-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$/.test(key)) {
    sendToClient(ws, { type: 'system', data: { message: '卡密格式错误！格式: VIP-XXXX-XXXX-XXXX' } });
    return;
  }
  
  // Load VIP keys
  let vipKeys = {};
  try { vipKeys = readJSON(VIP_KEYS_FILE); } catch(e) { vipKeys = {}; }
  
  const keyData = vipKeys[key];
  if (!keyData) {
    sendToClient(ws, { type: 'system', data: { message: '无效的卡密！' } });
    return;
  }
  if (keyData.usedBy) {
    sendToClient(ws, { type: 'system', data: { message: '该卡密已被使用！' } });
    return;
  }
  
  const config = VIP_CONFIG[keyData.type];
  if (!config) {
    sendToClient(ws, { type: 'system', data: { message: '未知的卡密类型！' } });
    return;
  }
  ...
  const p = players[player.id];
  const now = Date.now();
  const addDays = config.days * 86400000;
  
  // If already VIP, extend; otherwise activate
  if (p.vipExpiry && p.vipExpiry > now) {
    p.vipExpiry += addDays;
  } else {
    p.vipExpiry = now + addDays;
  }
  p.vipType = keyData.type;
  p.vipTitle = config.title;
  
  // Mark key as used
  keyData.usedBy = player.id;
  keyData.usedAt = now;
  vipKeys[key] = keyData;
  writeJSON(VIP_KEYS_FILE, vipKeys);
  
  savePlayers(players);
  sendToClient(ws, {
    type: 'system',
    data: { message: `🎉 VIP激活成功！\n类型: ${config.name}\n称号: ${config.title}\n有效期: ${config.days}天\n效果: 经验+20%, 灵石+10%, 签到额外丹药` }
  });
}

// /生成卡密 <类型> <数量> - 管理员生成VIP卡密
function handleGenerateVipKeyCommand(ws, player, command) {
  if (!player.isAdmin) {
    sendToClient(ws, { type: 'system', data: { message: '你没有管理员权限。' } });
    return;
  }
  
  const args = command.replace('/生成卡密', '').trim().split(/\s+/);
  const type = args[0];
  const count = Math.min(parseInt(args[1]) || 1, 20);
  
  if (!VIP_CONFIG[type]) {
    sendToClient(ws, { type: 'system', data: { message: '用法: /生成卡密 <类型> <数量>\n类型: week(周卡), month(月卡), year(年卡)' } });
    return;
  }
  ...
  for (let i = 0; i < count; i++) {
    const keyData = generateVipKey(type);
    vipKeys[keyData.code] = keyData;
    generated.push(keyData.code);
  }
  ...
  sendToClient(ws, {
    ...
    data: {
      title: '生成卡密',
      content: `\n╔══════════════════════════════════╗\n║  已生成 ${count} 个${config.name}卡密\n╠══════════════════════════════════╣\n${generated.map(c => `║  ${c}`).join('\n')}\n╚══════════════════════════════════╝`
    }
  });
}

// /解谜 - 遗物排列解谜系统
function handlePuzzleCommand(ws, player) {
  const players = getPlayers();
  const p = players[player.id];
  
  // Check if already has a puzzle in progress
  if (p.puzzleProgress) {
    const puzzle = p.puzzleProgress;
    const allRelics = Object.keys(RELICS_DATA);
    const correct = puzzle.arrangement;
    let matchCount = 0;
    for (let i = 0; i < correct.length; i++) {
      if (correct[i] === puzzle.playerArrangement[i]) matchCount++;
    }
    sendToClient(ws, {
      type: 'command_response',
      data: {
        title: '遗物解谜',
        content: `\n╔══════════════════════════════════╗\n║  遗物排列解谜 (进行中)\n╠══════════════════════════════════╣\n║  已尝试: ${p.puzzleAttempts}次\n║  当前匹配: ${matchCount}/36\n║\n║  使用 /排列 <序号1> <序号2> ... 排列\n║  使用 /谜题提示 获取提示 (100灵石)\n║  使用 /放弃谜题 放弃当前谜题\n╚══════════════════════════════════╝`
      }
    });
    return;
  }
  
  // Start new puzzle - costs 50 silver
  if ((p.silver || 0) < 50) {
    sendToClient(ws, { type: 'system', data: { message: '灵石不足！解谜需要50灵石。' } });
    return;
  }
  
  p.silver -= 50;
  
  // Generate puzzle: random arrangement of all 36 relics
  const allRelics = Object.keys(RELICS_DATA);
  const arrangement = [...allRelics].sort(() => Math.random() - 0.5);
  
  p.puzzleProgress = {
    arrangement: arrangement,
    playerArrangement: new Array(36).fill(null),
    startTime: Date.now()
  };
  p.puzzleAttempts = (p.puzzleAttempts || 0) + 1;
  ...
  sendToClient(ws, {
    ...
    data: {
      title: '遗物解谜',
      content: `\n╔══════════════════════════════════╗\n║  遗物排列解谜 (已开始)\n╠══════════════════════════════════╣\n║  将36件遗物按正确顺序排列！\n║\n║  遗物列表 (按puzzleWeight排序):\n║  十二月: M01-M12\n║  十二时辰: T01-T12\n║  十二生肖: Z01-Z12\n║\n║  正确顺序: puzzleWeight从小到大\n║  即: M01,M02,...M12,T01,...T12,Z01,...Z12\n║\n║  消耗: 50灵石\n║  奖励: 大量经验+手表经验\n║\n║  使用 /谜题提示 获取提示 (100灵石)\n╚══════════════════════════════════╝`
    }
  });
}

// /谜题提示 - 获取解谜提示
function handlePuzzleHintCommand(ws, player) {
  const players = getPlayers();
  const p = players[player.id];
  
  if (!p.puzzleProgress) {
    sendToClient(ws, { type: 'system', data: { message: '你没有正在进行的谜题。使用 /解谜 开始。' } });
    return;
  }
  
  if ((p.silver || 0) < 100) {
    sendToClient(ws, { type: 'system', data: { message: '灵石不足！提示需要100灵石。' } });
    return;
  }
  
  p.silver -= 100;
  
  // Give a hint: reveal 3 positions
  const puzzle = p.puzzleProgress;
  const hintPositions = [];
  for (let i = 0; i < 3; i++) {
    const pos = Math.floor(Math.random() * 36);
    if (!hintPositions.includes(pos)) hintPositions.push(pos);
  }
  
  const hints = hintPositions.map(pos => {
    const relic = RELICS_DATA[puzzle.arrangement[pos]];
    return `位置${pos + 1}: ${relic ? relic.name : '未知'}`;
  });
  
  savePlayers(players);
  sendToClient(ws, {
    type: 'system',
    data: { message: `💡 谜题提示 (消耗100灵石):\n${hints.join('\n')}\n\n正确排列: M01→M12, T01→T12, Z01→Z12` }
  });
}

// /修复Bug <BugID> - 修复Bug获得手表经验
function handleFixBugCommand(ws, player, bugId) {
  if (!bugId) {
    sendToClient(ws, { type: 'system', data: { message: '用法: /修复Bug <BugID>\n输入 /Bug 查看已发现的Bug。' } });
    return;
  }
  
  bugId = bugId.toUpperCase();
  const bd = BUGS_DATA[bugId];
  if (!bd) {
    sendToClient(ws, { type: 'system', data: { message: `未知Bug: ${bugId}` } });
    return;
  }
  ...
  // Must have discovered the bug
  if (!(p.discoveredBugs || []).includes(bugId)) {
    sendToClient(ws, { type: 'system', data: { message: `你还没有发现Bug【${bd.name}】。使用 /探索Bug 搜索。` } });
    return;
  }
  
  // Check if already fixed
  if ((p.fixedBugs || []).includes(bugId)) {
    sendToClient(ws, { type: 'system', data: { message: `Bug【${bd.name}】已经修复过了！` } });
    return;
  }
  
  // Check fix requirement
  const fixReq = bd.fixReq || { gold: 100 };
  const silverCost = fixReq.gold || 100;
  if ((p.silver || 0) < silverCost) {
    sendToClient(ws, { type: 'system', data: { message: `修复Bug需要${silverCost}灵石，你灵石不足。` } });
    return;
  }
  
  p.silver -= silverCost;
  if (!p.fixedBugs) p.fixedBugs = [];
  p.fixedBugs.push(bugId);
  
  // Give watch exp and regular exp
  const watchExpGain = 30;
  const expGain = 200;
  p.watchExp = (p.watchExp || 0) + watchExpGain;
  
  // Update watch level
  if (p.adminWatch) {
    const oldLevel = p.adminWatchLevel || 0;
    const newLevel = calcWatchLevel(p.watchExp);
    if (newLevel > oldLevel) {
      p.adminWatchLevel = newLevel;
      sendToClient(ws, {
        type: 'system',
        data: { message: `⌚ 管理员手表升级！Lv${oldLevel} → Lv${newLevel}\n新能力: ${WATCH_THRESHOLDS[newLevel].abilities}` }
      });
    }
  }
  ...
  sendToClient(ws, {
    type: 'system',
    data: { message: `🔧 Bug修复成功！\n修复: 【${bd.name}】(${bd.desc})\n消耗: ${silverCost}灵石\n获得: ${expGain}经验, ${watchExpGain}手表经验\n已修复: ${(p.fixedBugs || []).length}/${ALL_BUG_IDS.length}` }
  });
}

// /结局进度 - 查看真结局完成度
function handleEndingProgressCommand(ws, player) {
  const p = player;
  const relics = p.relics || [];
  const totalRelics = Object.keys(RELICS_DATA).length; // 36
  const watchLevel = p.adminWatchLevel || 0;
  const fixedBugs = (p.fixedBugs || []).length;
  const totalBugs = ALL_BUG_IDS.length; // 32
  const daoHeart = p.daoHeart || 500;
  const mainQuestComplete = p.mainQuestComplete || false;
  
  // Condition A: 成神
  const condA_progress = [
    { name: '集齐36遗物', done: relics.length >= totalRelics, current: `${relics.length}/${totalRelics}` },
    { name: '手表Lv9', done: watchLevel >= 9, current: `Lv${watchLevel}/Lv9` },
    { name: '修复32Bug', done: fixedBugs >= totalBugs, current: `${fixedBugs}/${totalBugs}` }
  ];
  const condA_done = condA_progress.every(c => c.done);
  
  // Condition B: 回归
  const condB_progress = [
    { name: '完成主线剧情', done: mainQuestComplete, current: mainQuestComplete ? '已完成' : '未完成' },
    { name: '道心1000', done: daoHeart >= 1000, current: `${daoHeart}/1000` }
  ];
  const condB_done = condB_progress.every(c => c.done);
  
  let content = `\n╔══════════════════════════════════╗\n║  真结局进度\n╠══════════════════════════════════╣\n║  条件A - 成神之路:\n`;
  for (const c of condA_progress) {
    content += `║    ${c.done ? '✅' : '🔒'} ${c.name}: ${c.current}\n`;
  }
  content += `║  状态: ${condA_done ? '✅ 可选择' : '❌ 未达成'}\n`;
  content += `╠══════════════════════════════════╣\n║  条件B - 回归之路:\n`;
  for (const c of condB_progress) {
    content += `║    ${c.done ? '✅' : '🔒'} ${c.name}: ${c.current}\n`;
  }
  content += `║  状态: ${condB_done ? '✅ 可选择' : '❌ 未达成'}\n`;
  content += `╠══════════════════════════════════╣\n║  ${p.endingChoice ? '已选择: ' + p.endingChoice : '尚未选择结局'}\n`;
  if (condA_done || condB_done) {
    content += `║  使用 /选择结局 触发真结局\n`;
  }
  content += `╚══════════════════════════════════╝`;
  
  sendToClient(ws, {
    type: 'command_response',
    data: { title: '真结局进度', content }
  });
}

// /选择结局 - 触发真结局
function handleChooseEndingCommand(ws, player) {
  const p = player;
  const relics = p.relics || [];
  const totalRelics = Object.keys(RELICS_DATA).length;
  const watchLevel = p.adminWatchLevel || 0;
  const fixedBugs = (p.fixedBugs || []).length;
  const totalBugs = ALL_BUG_IDS.length;
  const daoHeart = p.daoHeart || 500;
  
  const condA = relics.length >= totalRelics && watchLevel >= 9 && fixedBugs >= totalBugs;
  const condB = (p.mainQuestComplete || false) && daoHeart >= 1000;
  
  if (!condA && !condB) {
    sendToClient(ws, { type: 'system', data: { message: '你还没有达成任何真结局条件。使用 /结局进度 查看。' } });
    return;
  }
  
  if (p.endingChoice) {
    sendToClient(ws, { type: 'system', data: { message: `你已经选择了结局【${p.endingChoice}】，无法更改。` } });
    return;
  }
  
  const players = getPlayers();
  const pl = players[player.id];
  
  let endingMsg = '';
  if (condA && condB) {
    // Both conditions met - can choose
    pl.endingChoice = '成神(集大成者)';
    endingMsg = `🎊🎊🎊 真结局触发 — 集大成者！\n\n你集齐了所有遗物，掌握了创世者权限，修复了所有Bug，道心圆满。\n\n你成为了这个赛博世界的神。\n代码在你指尖流动，世界在你掌中旋转。\n\n「作为2030年的退休程序员，你终于理解了：\n  这个世界的本质，就是代码。」\n\n🏆 获得称号: 赛博创世者\n🏆 全属性+30%`;
    // Apply ending bonuses
    pl.maxHp = Math.floor(pl.maxHp * 1.3);
    pl.maxSpirit = Math.floor(pl.maxSpirit * 1.3);
  } else if (condA) {
    pl.endingChoice = '成神';
    endingMsg = `🎊🎊🎊 真结局触发 — 成神之路！\n\n你集齐了所有遗物，掌握了创世者权限，修复了所有Bug。\n\n你成为了赛博世界的管理员之神。\n代码在你指尖流动，万物在你掌控之中。\n\n「这个世界，由你来书写。」\n\n🏆 获得称号: 赛博之神\n🏆 全属性+20%`;
    pl.maxHp = Math.floor(pl.maxHp * 1.2);
    pl.maxSpirit = Math.floor(pl.maxSpirit * 1.2);
  } else {
    pl.endingChoice = '回归';
    endingMsg = `🎊🎊🎊 真结局触发 — 回归之路！\n\n你完成了主线剧情，道心圆满。\n\n你放下了所有遗物和Bug，回归了最初的自己。\n作为60岁的退休老人，你找到了内心的平静。\n\n「修仙不在于力量，而在于心。」\n\n🏆 获得称号: 归真者\n🏆 道心锁定1000, 寿命+50`;
    pl.daoHeart = 1000;
    pl.lifespan = (pl.lifespan || 100) + 50;
  }
  
  savePlayers(players);
  sendToClient(ws, {
    type: 'system',
    data: { message: endingMsg }
  });
  
  // Broadcast to all players
  broadcast({
    type: 'system',
    data: { message: `🌟 ${pl.name} 触发了真结局【${pl.endingChoice}】！世界为之震动！` }
  });
}
...
const idleTickInterval = setInterval(() => {
  for (const [ws, clientInfo] of connectedClients.entries()) {
    ...
    // Watch Lv4+: exp+10%, Lv5+: +15%
    if (player.adminWatch && player.adminWatchLevel >= 5) {
      idleExpAmount = Math.floor(idleExpAmount * 1.15);
    } else if (player.adminWatch && player.adminWatchLevel >= 4) {
      idleExpAmount = Math.floor(idleExpAmount * 1.1);
    }
    // Relic bonuses
    ...
    // VIP bonus: +20% exp
    if (isVipActive(player)) {
      idleExpAmount = Math.floor(idleExpAmount * (1 + getVipExpBonus(player)));
    }
    // Relic set bonus (hour type)
    const setBonus = getRelicSetBonus(player);
    if (setBonus.hour) idleExpAmount = Math.floor(idleExpAmount * (1 + setBonus.hour.pct / 100));
...
const ACHIEVEMENTS = {
  // ===== 战斗成就(15个) =====
  first_kill: { name: '初出茅庐', desc: '首次击杀怪物', icon: '⚔️', check: p => (p.totalKills || 0) >= 1 },
  kill_10: { name: '小试牛刀', desc: '击杀10只怪物', icon: '🗡️', check: p => (p.totalKills || 0) >= 10 },
  kill_50: { name: '身经百战', desc: '击杀50只怪物', icon: '⚔️', check: p => (p.totalKills || 0) >= 50 },
  kill_100: { name: '百战老兵', desc: '击杀100只怪物', icon: '🗡️', check: p => (p.totalKills || 0) >= 100 },
  kill_500: { name: '杀神', desc: '击杀500只怪物', icon: '💀', check: p => (p.totalKills || 0) >= 500 },
  kill_1000: { name: '千人斩', desc: '击杀1000只怪物', icon: '💀', check: p => (p.totalKills || 0) >= 1000 },
  boss_1: { name: 'Boss猎人', desc: '首次击杀世界Boss', icon: '👹', check: p => (p.bossKills || []).length >= 1 },
  boss_5: { name: 'Boss克星', desc: '击杀5次世界Boss', icon: '👹', check: p => (p.bossKills || []).length >= 5 },
  boss_all: { name: 'Boss终结者', desc: '击杀所有世界Boss', icon: '👑', check: p => (p.bossKills || []).length >= 5 },
  pvp_1: { name: '初尝鲜血', desc: '首次PvP击杀', icon: '🩸', check: p => (p.pvpKills || 0) >= 1 },
  pvp_10: { name: 'PvP狂人', desc: 'PvP击杀10次', icon: '🩸', check: p => (p.pvpKills || 0) >= 10 },
  perfect_kill: { name: '完美击杀', desc: '无伤击杀Boss', icon: '✨', check: p => p.perfectBossKill || false },
  speed_kill: { name: '闪电战', desc: '1回合内击杀怪物', icon: '⚡', check: p => p.speedKill || false },
  counter_kill: { name: '绝地反击', desc: 'HP低于10%时反杀', icon: '🔥', check: p => p.counterKill || false },
  combo_kill: { name: '连击大师', desc: '连续击杀10只怪物不休息', icon: '💥', check: p => p.comboKill >= 10 },
  // ===== 修炼成就(15个) =====
  realm_1: { name: '练气入门', desc: '达到练气境', icon: '🌀', check: p => (p.realmLevel || 0) >= 1 },
  realm_2: { name: '筑基成功', desc: '达到筑基境', icon: '🌀', check: p => (p.realmLevel || 0) >= 2 },
  realm_3: { name: '金丹大道', desc: '达到金丹境', icon: '💎', check: p => (p.realmLevel || 0) >= 3 },
  realm_4: { name: '元婴化形', desc: '达到元婴境', icon: '💎', check: p => (p.realmLevel || 0) >= 4 },
  realm_5: { name: '化神通天', desc: '达到化神境', icon: '⚡', check: p => (p.realmLevel || 0) >= 5 },
  realm_6: { name: '炼虚合道', desc: '达到炼虚境', icon: '⚡', check: p => (p.realmLevel || 0) >= 6 },
  realm_7: { name: '合体大成', desc: '达到合体境', icon: '🌌', check: p => (p.realmLevel || 0) >= 7 },
  realm_8: { name: '大乘圆满', desc: '达到大乘境', icon: '🌌', check: p => (p.realmLevel || 0) >= 8 },
  realm_9: { name: '渡劫飞升', desc: '达到渡劫境', icon: '🌟', check: p => (p.realmLevel || 0) >= 9 },
  level_10: { name: '十级修士', desc: '达到10级', icon: '📈', check: p => (p.level || 1) >= 10 },
  level_30: { name: '三十而立', desc: '达到30级', icon: '📈', check: p => (p.level || 1) >= 30 },
  level_50: { name: '半百修士', desc: '达到50级', icon: '📈', check: p => (p.level || 1) >= 50 },
  level_80: { name: '耄耋修士', desc: '达到80级', icon: '📈', check: p => (p.level || 1) >= 80 },
  skill_10: { name: '功法小成', desc: '学会10种功法', icon: '📚', check: p => (p.skills || []).length >= 10 },
  skill_master: { name: '功法大成', desc: '功法总熟练度达到1000', icon: '📚', check: p => (p.skills || []).reduce((s, sk) => s + (sk.exp || 0), 0) >= 1000 },
  // ===== 收集成就(15个) =====
  relic_1: { name: '遗物初见', desc: '收集1件遗物', icon: '📦', check: p => (p.relics || []).length >= 1 },
  relic_6: { name: '遗物收藏者', desc: '收集6件遗物', icon: '📦', check: p => (p.relics || []).length >= 6 },
  relic_12: { name: '遗物收藏家', desc: '收集12件遗物', icon: '📦', check: p => (p.relics || []).length >= 12 },
  relic_24: { name: '遗物大师', desc: '收集24件遗物', icon: '🏆', check: p => (p.relics || []).length >= 24 },
  relic_36: { name: '遗物传说', desc: '集齐36件遗物', icon: '🏆', check: p => (p.relics || []).length >= 36 },
  bug_1: { name: 'Bug初见', desc: '发现1种Bug', icon: '🐛', check: p => (p.discoveredBugs || []).length >= 1 },
  bug_8: { name: 'Bug猎人', desc: '发现8种Bug', icon: '🐛', check: p => (p.discoveredBugs || []).length >= 8 },
  bug_16: { name: 'Bug专家', desc: '发现16种Bug', icon: '🐛', check: p => (p.discoveredBugs || []).length >= 16 },
  bug_32: { name: 'Bug终结者', desc: '发现全部32种Bug', icon: '🔧', check: p => (p.discoveredBugs || []).length >= 32 },
  equip_full: { name: '全副武装', desc: '穿戴满所有装备位', icon: '🛡️', check: p => p.equipment && Object.keys(p.equipment).length >= 5 },
  pill_10: { name: '丹药收藏者', desc: '拥有10种不同丹药', icon: '💊', check: p => Object.keys(p.pills || {}).length >= 10 },
  material_20: { name: '材料收藏家', desc: '拥有20种不同材料', icon: '🧪', check: p => Object.keys(p.materials || {}).length >= 20 },
  treasure_1: { name: '法宝初成', desc: '获得1件法宝', icon: '🏮', check: p => (p.treasures || []).length >= 1 },
  treasure_5: { name: '法宝收藏家', desc: '获得5件法宝', icon: '🏮', check: p => (p.treasures || []).length >= 5 },
  sect_shop: { name: '门派常客', desc: '在门派商店购买10次', icon: '🏯', check: p => (p.sectShopBuy || 0) >= 10 },
  // ===== 社交成就(15个) =====
  friend_1: { name: '初识好友', desc: '拥有1个好友', icon: '🤝', check: p => (p.friends || []).length >= 1 },
  friend_5: { name: '广结善缘', desc: '拥有5个好友', icon: '🤝', check: p => (p.friends || []).length >= 5 },
  friend_10: { name: '社交达人', desc: '拥有10个好友', icon: '🤝', check: p => (p.friends || []).length >= 10 },
  sect_join: { name: '拜师学艺', desc: '加入门派', icon: '🏯', check: p => !!p.sect },
  sect_master: { name: '一派之主', desc: '成为门派掌门', icon: '👑', check: p => p.sectPosition === '掌门' },
  mentor_1: { name: '拜师学艺', desc: '拜一位师父', icon: '👨‍🏫', check: p => !!p.mentor },
  apprentice_1: { name: '收徒传道', desc: '收一位徒弟', icon: '👨‍🎓', check: p => (p.apprentices || []).length >= 1 },
  sworn_1: { name: '结义之交', desc: '与人结义', icon: '🤝', check: p => (p.swornBrothers || []).length >= 1 },
  trade_1: { name: '初次交易', desc: '完成1次交易', icon: '💰', check: p => (p.tradeCount || 0) >= 1 },
  trade_10: { name: '交易达人', desc: '完成10次交易', icon: '💰', check: p => (p.tradeCount || 0) >= 10 },
  auction_1: { name: '拍卖新手', desc: '首次上架拍卖', icon: '🔨', check: p => (p.auctionCount || 0) >= 1 },
  bounty_1: { name: '赏金猎人', desc: '完成1次悬赏', icon: '🏴', check: p => (p.bountyCount || 0) >= 1 },
  chat_100: { name: '话痨', desc: '发送100条消息', icon: '💬', check: p => (p.chatCount || 0) >= 100 },
  team_1: { name: '组队达人', desc: '组队战斗10次', icon: '👥', check: p => (p.teamBattles || 0) >= 10 },
  grudge_1: { name: '仇人见面', desc: '拥有1个仇人', icon: '⚔️', check: p => (p.grudges || []).length >= 1 },
  // ===== 财富成就(10个) =====
  silver_100: { name: '小有积蓄', desc: '拥有100灵石', icon: '💰', check: p => (p.silver || 0) >= 100 },
  silver_1000: { name: '家财万贯', desc: '拥有1000灵石', icon: '💰', check: p => (p.silver || 0) >= 1000 },
  silver_10000: { name: '富甲一方', desc: '拥有10000灵石', icon: '💰', check: p => (p.silver || 0) >= 10000 },
  silver_100000: { name: '富可敌国', desc: '拥有100000灵石', icon: '💎', check: p => (p.silver || 0) >= 100000 },
  silver_1000000: { name: '灵石大亨', desc: '拥有1000000灵石', icon: '💎', check: p => (p.silver || 0) >= 1000000 },
  silver_spent_10000: { name: '挥金如土', desc: '累计花费10000灵石', icon: '💸', check: p => (p.totalSilverSpent || 0) >= 10000 },
  silver_earned_10000: { name: '勤劳致富', desc: '累计获得10000灵石', icon: '💪', check: p => (p.totalSilverEarned || 0) >= 10000 },
  shop_buy_50: { name: '购物狂', desc: '在商店购买50次', icon: '🛒', check: p => (p.shopBuyCount || 0) >= 50 },
  shop_sell_50: { name: '卖家达人', desc: '在商店出售50次', icon: '🏪', check: p => (p.shopSellCount || 0) >= 50 },
  lucky_dog: { name: '天降横财', desc: '奇遇获得1000灵石', icon: '🍀', check: p => p.luckySilver || false },
  // ===== 特殊成就(15个) =====
  vip_achieve: { name: '尊贵会员', desc: '激活VIP', icon: '🌟', check: p => !!p.vipExpiry && p.vipExpiry > Date.now() },
  watch_lv1: { name: '手表入门', desc: '手表达到Lv1', icon: '⌚', check: p => (p.adminWatchLevel || 0) >= 1 },
  watch_lv5: { name: '手表精通', desc: '手表达到Lv5', icon: '⌚', check: p => (p.adminWatchLevel || 0) >= 5 },
  watch_lv9: { name: '创世者', desc: '手表达到Lv9', icon: '⌚', check: p => (p.adminWatchLevel || 0) >= 9 },
  puzzle_solve: { name: '谜题大师', desc: '完成遗物解谜', icon: '🧩', check: p => !!p.puzzleSolved },
  ending_a: { name: '成神之路', desc: '达成结局A', icon: '🎊', check: p => p.endingChoice === 'A' },
  ending_b: { name: '回归之路', desc: '达成结局B', icon: '🎊', check: p => p.endingChoice === 'B' },
  dao_500: { name: '道心初成', desc: '道心达到500', icon: '❤️', check: p => (p.daoHeart || 0) >= 500 },
  dao_800: { name: '道心稳固', desc: '道心达到800', icon: '❤️', check: p => (p.daoHeart || 0) >= 800 },
  dao_1000: { name: '道心圆满', desc: '道心达到1000', icon: '❤️', check: p => (p.daoHeart || 0) >= 1000 },
  credit_800: { name: '信誉良好', desc: '信誉达到800', icon: '📊', check: p => (p.creditScore || 0) >= 800 },
  sign_7: { name: '签到达人', desc: '连续签到7天', icon: '📅', check: p => (p.signConsecutive || 0) >= 7 },
  sign_30: { name: '坚持不懈', desc: '连续签到30天', icon: '📅', check: p => (p.signConsecutive || 0) >= 30 },
  sign_100: { name: '签到传说', desc: '连续签到100天', icon: '📅', check: p => (p.signConsecutive || 0) >= 100 },
  explore_all: { name: '世界探索者', desc: '探索所有14张地图', icon: '🗺️', check: p => Object.keys(p.exploredMaps || {}).length >= 14 }
};
...
function checkAchievements(player) {
  ...
  for (const [id, ach] of Object.entries(ACHIEVEMENTS)) {
    if (!player.achievements.includes(id) && ach.check(player)) {
...
function handleAchievementCommand(ws, player) {
  const players = getPlayers();
  const p = players[player.id];
  const newAch = checkAchievements(p);
  if (newAch.length > 0) savePlayers(players);
  
  const unlocked = p.achievements || [];
  let content = '\n╔══════════════════════════════════╗\n║  成就系统\n╠══════════════════════════════════╣\n';
  content += `║  已解锁: ${unlocked.length}/${Object.keys(ACHIEVEMENTS).length}\n╠══════════════════════════════════╣\n`;
  for (const [id, ach] of Object.entries(ACHIEVEMENTS)) {
    const done = unlocked.includes(id);
    content += `║  ${done ? ach.icon : '🔒'} ${ach.name}: ${ach.desc} ${done ? '✅' : ''}\n`;
  }
  content += '╚══════════════════════════════════╝';
  
  sendToClient(ws, { type: 'command_response', data: { title: '成就系统', content } });
  
  if (newAch.length > 0) {
    const names = newAch.map(a => `${a.icon} ${a.name}`).join(', ');
    sendToClient(ws, { type: 'system', data: { message: `🎉 新成就解锁: ${names}` } });
  }
}

// ===== LEADERBOARD SYSTEM (排行榜) =====
function handleLeaderboardCommand(ws, player) {
  const players = getPlayers();
  const sorted = Object.values(players).sort((a, b) => (b.level || 1) - (a.level || 1));
  const top10 = sorted.slice(0, 10);
  
  let content = '\n╔══════════════════════════════════╗\n║  排行榜 (Top 10)\n╠══════════════════════════════════╣\n';
  top10.forEach((p, i) => {
    const medal = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i+1}.`;
    content += `║  ${medal} ${p.name} Lv${p.level} ${p.realm || ''}\n`;
  });
  
  const playerRank = sorted.findIndex(p => p.id === player.id) + 1;
  content += `╠══════════════════════════════════╣\n║  你的排名: #${playerRank}\n╚══════════════════════════════════╝`;
  
  sendToClient(ws, { type: 'command_response', data: { title: '排行榜', content } });
}

// ===== BOSS SYSTEM (BOSS战) =====
const WORLD_BOSSES = [
  { id: 'boss_1', name: '千年树妖', level: 20, hp: 5000, attack: 80, defense: 40, exp: 2000, silver: 500, drop: '妖兽内丹', dropRate: 0.3, realm: '竹林深处' },
  { id: 'boss_2', name: '洛阳城主', level: 30, hp: 10000, attack: 150, defense: 80, exp: 5000, silver: 1500, drop: '城主令牌', dropRate: 0.2, realm: '洛阳城' },
  { id: 'boss_3', name: '护山神兽', level: 40, hp: 20000, attack: 250, defense: 120, exp: 10000, silver: 3000, drop: '神兽之血', dropRate: 0.15, realm: '正派山门' },
  { id: 'boss_4', name: '邪派魔尊', level: 40, hp: 20000, attack: 280, defense: 100, exp: 10000, silver: 3000, drop: '魔尊之眼', dropRate: 0.15, realm: '邪派禁地' },
  { id: 'boss_5', name: '秘境妖王', level: 50, hp: 35000, attack: 400, defense: 200, exp: 20000, silver: 6000, drop: '妖王内丹', dropRate: 0.12, realm: '修仙秘境' },
  { id: 'boss_6', name: '远古神兽', level: 60, hp: 50000, attack: 600, defense: 300, exp: 40000, silver: 12000, drop: '神兽晶核', dropRate: 0.10, realm: '上古遗迹' },
  { id: 'boss_7', name: '天劫雷兽', level: 70, hp: 80000, attack: 900, defense: 450, exp: 80000, silver: 25000, drop: '雷劫精华', dropRate: 0.08, realm: '渡劫仙域' },
  { id: 'boss_8', name: '源码守护者', level: 80, hp: 100000, attack: 1200, defense: 600, exp: 150000, silver: 50000, drop: '源码碎片', dropRate: 0.05, realm: '代码空间' },
  { id: 'boss_9', name: '水乡龙王', level: 35, hp: 15000, attack: 200, defense: 100, exp: 8000, silver: 2500, drop: '龙王鳞片', dropRate: 0.18, realm: '江南水乡' },
  { id: 'boss_10', name: '沙漠沙虫', level: 45, hp: 25000, attack: 350, defense: 180, exp: 15000, silver: 5000, drop: '沙虫晶核', dropRate: 0.12, realm: '沙漠戈壁' },
  { id: 'boss_11', name: '冰霜巨龙', level: 55, hp: 45000, attack: 550, defense: 280, exp: 30000, silver: 10000, drop: '冰龙之心', dropRate: 0.10, realm: '北境冰原' },
  { id: 'boss_12', name: '蛊虫之母', level: 55, hp: 40000, attack: 500, defense: 250, exp: 28000, silver: 9000, drop: '蛊母精华', dropRate: 0.10, realm: '南疆密林' },
  { id: 'boss_13', name: '海底巨兽', level: 65, hp: 70000, attack: 800, defense: 400, exp: 60000, silver: 20000, drop: '巨兽之牙', dropRate: 0.08, realm: '海底龙宫' },
  { id: 'boss_14', name: '虚空帝王', level: 80, hp: 150000, attack: 1500, defense: 800, exp: 200000, silver: 80000, drop: '虚空帝王之冠', dropRate: 0.03, realm: '虚空裂缝' }
];

function handleBossCommand(ws, player, command) {
  const arg = command.replace('/BOSS', '').trim();
  if (!arg) {
    let content = '\n╔══════════════════════════════════╗\n║  世界BOSS\n╠══════════════════════════════════╣\n';
    for (const boss of WORLD_BOSSES) {
      const killed = (player.bossKills || {})[boss.id] || 0;
      content += `║  ${boss.name} Lv${boss.level}\n║    位置: ${boss.realm} | 击杀: ${killed}次\n║    奖励: ${boss.exp}经验, ${boss.silver}灵石\n║    掉落: ${boss.drop}(${Math.floor(boss.dropRate*100)}%)\n║\n`;
    }
    content += '║  /BOSS 挑战 <BOSS名> - 挑战BOSS\n╚══════════════════════════════════╝';
    sendToClient(ws, { type: 'command_response', data: { title: '世界BOSS', content } });
    return;
  }
  
  if (arg.startsWith('挑战')) {
    const bossName = arg.replace('挑战', '').trim();
    const boss = WORLD_BOSSES.find(b => b.name === bossName);
    if (!boss) { sendToClient(ws, { type: 'system', data: { message: '未找到该BOSS。' } }); return; }
    if ((player.level || 1) < boss.level - 10) {
      sendToClient(ws, { type: 'system', data: { message: `你的等级不足！需要至少Lv${boss.level - 10}才能挑战${boss.name}。` } });
      return;
    }
    // Simple boss combat
    const players = getPlayers();
    const p = players[player.id];
    let playerHp = p.hp || p.maxHp || 100;
    let bossHp = boss.hp;
    let rounds = 0;
    const log = [];
    while (playerHp > 0 && bossHp > 0 && rounds < 50) {
      rounds++;
      const pAtk = Math.max(1, (p.attack || 50) - boss.defense + Math.floor(Math.random() * 20));
      bossHp -= pAtk;
      log.push(`第${rounds}回合: 你攻击${boss.name}造成${pAtk}伤害 (BOSS HP: ${Math.max(0, bossHp)})`);
      if (bossHp <= 0) break;
      const bAtk = Math.max(1, boss.attack - (p.defense || 20) + Math.floor(Math.random() * 20));
      playerHp -= bAtk;
      log.push(`第${rounds}回合: ${boss.name}攻击你造成${bAtk}伤害 (你的HP: ${Math.max(0, playerHp)})`);
    }
    
    if (bossHp <= 0) {
      // Victory
      p.exp = (p.exp || 0) + boss.exp;
      p.silver = (p.silver || 0) + boss.silver;
      if (!p.bossKills) p.bossKills = {};
      p.bossKills[boss.id] = (p.bossKills[boss.id] || 0) + 1;
      let dropMsg = '';
      if (Math.random() < boss.dropRate) {
        if (!p.materials) p.materials = {};
        p.materials[boss.drop] = (p.materials[boss.drop] || 0) + 1;
        dropMsg = `\n║  获得掉落: ${boss.drop}`;
      }
      savePlayers(players);
      log.push(`\n🎉 你击败了${boss.name}！\n║  获得: ${boss.exp}经验, ${boss.silver}灵石${dropMsg}`);
    } else {
      p.hp = Math.floor((p.maxHp || 100) * 0.3);
      savePlayers(players);
      log.push(`\n💀 你被${boss.name}击败了！HP恢复到30%。`);
    }
    
    sendToClient(ws, { type: 'command_response', data: { title: `BOSS战: ${boss.name}`, content: log.join('\n') } });
    return;
  }
}

// ===== MAIN QUEST SYSTEM (剧情主线) =====
const MAIN_QUESTS = [
  { id: 'mq_1', name: '第一章：初入江湖', desc: '完成新手引导，达到灵动境', condition: p => (p.realmLevel || 0) >= 3, reward: { exp: 500, silver: 200 } },
  { id: 'mq_2', name: '第二章：拜师学艺', desc: '加入门派，学习3个功法', condition: p => p.sect && (p.skills || []).length >= 3, reward: { exp: 1000, silver: 500 } },
  { id: 'mq_3', name: '第三章：历练江湖', desc: '击杀100只怪物，收集5件遗物', condition: p => (p.totalKills || 0) >= 100 && (p.relics || []).length >= 5, reward: { exp: 2000, silver: 1000 } },
  { id: 'mq_4', name: '第四章：门派纷争', desc: '成为门派长老，道心达到700', condition: p => ['长老', '掌门'].includes(p.sectPosition) && (p.daoHeart || 500) >= 700, reward: { exp: 5000, silver: 3000 } },
  { id: 'mq_5', name: '第五章：天劫降临', desc: '达到渡劫境，修复10种Bug', condition: p => (p.realmLevel || 0) >= 9 && (p.fixedBugs || []).length >= 10, reward: { exp: 10000, silver: 5000 } },
  { id: 'mq_6', name: '第六章：源码之心', desc: '达到最高境界，集齐36遗物', condition: p => (p.realmLevel || 0) >= 12 && (p.relics || []).length >= 36, reward: { exp: 50000, silver: 20000 }, final: true },
];

function handleMainQuestCommand(ws, player, command) {
  const arg = command.replace('/主线', '').trim();
  const players = getPlayers();
  const p = players[player.id];
  if (!p.mainQuestProgress) p.mainQuestProgress = 0;
  
  if (!arg || arg === '查看') {
    let content = '\n╔══════════════════════════════════╗\n║  剧情主线\n╠══════════════════════════════════╣\n';
    for (let i = 0; i < MAIN_QUESTS.length; i++) {
      const q = MAIN_QUESTS[i];
      const status = i < p.mainQuestProgress ? '✅ 已完成' : i === p.mainQuestProgress ? '📌 进行中' : '🔒 未解锁';
      content += `║  ${status} ${q.name}\n║    ${q.desc}\n║\n`;
    }
    content += `║  进度: ${p.mainQuestProgress}/${MAIN_QUESTS.length}\n╚══════════════════════════════════╝`;
    sendToClient(ws, { type: 'command_response', data: { title: '剧情主线', content } });
    return;
  }
  
  if (arg === '提交') {
    const current = MAIN_QUESTS[p.mainQuestProgress];
    if (!current) { sendToClient(ws, { type: 'system', data: { message: '所有主线任务已完成！' } }); return; }
    if (!current.condition(p)) { sendToClient(ws, { type: 'system', data: { message: `未完成条件: ${current.desc}` } }); return; }
    p.exp = (p.exp || 0) + current.reward.exp;
    p.silver = (p.silver || 0) + current.reward.silver;
    p.mainQuestProgress++;
    if (current.final) p.mainQuestComplete = true;
    savePlayers(players);
    sendToClient(ws, { type: 'system', data: { message: `🎉 完成【${current.name}】！\n获得: ${current.reward.exp}经验, ${current.reward.silver}灵石${current.final ? '\n🎊 主线剧情完结！' : ''}` } });
    return;
  }
}

// ===== NPC DIALOG SYSTEM (NPC对话) =====
const NPC_DIALOGS = {
  '村长': {
    greeting: '年轻人，欢迎来到赛博修仙的世界。这里的一切都是代码构成的。',
    topics: {
      '修仙': '修仙之路漫漫，需要不断修炼功法、提升境界。记住，道心是根本。',
      '遗物': '程序遗物是上古程序员留下的神器，集齐它们可以获得强大的力量。',
      'Bug': 'Bug是这个世界的漏洞，利用它们可以获得优势，但要小心被管理员发现。',
      '手表': '管理员手表是创世者的遗物，提升它的等级可以解锁更多功能。'
    }
  },
  '药铺老板': {
    greeting: '客官，需要什么丹药？本店货真价实！',
    topics: {
      '丹药': '丹药分回复、增益、特殊三类。战斗前准备好丹药很重要。',
      '炼丹': '想要自己炼丹？需要炼丹炉和材料。材料可以从怪物掉落或采集获得。',
      '材料': '灵草在竹林可以找到，妖兽内丹需要击杀BOSS获得。'
    }
  },
  '铁匠': {
    greeting: '要打造装备吗？给我材料就行！',
    topics: {
      '装备': '装备分武器、护甲、饰品三类，可以强化提升属性。',
      '强化': '装备强化需要强化石，强化等级越高，属性越强。',
      '锻造': '高级装备需要稀有材料，多去危险区域探索吧。'
    }
  },
  '洛阳城守卫': {
    greeting: '站住！洛阳城可不是随便什么人都能进的。',
    topics: { '进城': '出示你的身份令牌，或者...交点进城费。', '江湖': '最近江湖不太平，正邪两派又要打起来了。', '异象': '你说那些奇怪的现象？别乱说，小心被当成妖言惑众。' }
  },
  '神秘老者': {
    greeting: '年轻人，你身上有一股...特殊的气息。',
    topics: { '修炼': '修仙之道，在于悟。悟透了，境界自然提升。', '遗物': '遗物？你竟然知道遗物...看来你不是普通人。', '真相': '这个世界的真相...等你准备好了，自然会知道。', '代码': '代码？你怎么会知道这个词...（老者脸色大变）' }
  },
  '黑市情报贩子': {
    greeting: '想知道什么？价格好商量。',
    topics: { '正派': '正派那帮伪君子，表面光鲜，背地里...', '邪派': '邪派虽然名声不好，但至少坦荡。', '遗物': '遗物？我有线索，但价格不便宜。', 'Bug': 'Bug？你说的是那些\'异常\'？我见过，在代码空间里。' }
  },
  '少林方丈': {
    greeting: '阿弥陀佛，施主有何贵干？',
    topics: { '佛法': '佛法无边，修仙亦是修心。', '武学': '少林七十二绝技，每一门都博大精深。', '正邪': '正邪之分，在心不在名。', '创世者': '创世者...那是上古传说，不可轻信。' }
  },
  '武当掌门': {
    greeting: '道友，武当山欢迎你。',
    topics: { '道法': '道法自然，无为而治。', '剑法': '武当剑法，以柔克刚。', '正邪': '正邪之争，由来已久。', '秘境': '修仙秘境中，据说藏有上古遗物。' }
  },
  '东方教主': {
    greeting: '哼，又一个不知天高地厚的小子。',
    topics: { '日月神教': '日月神教，千秋万载，一统江湖！', '正派': '那帮伪君子，迟早要收拾他们。', '邪道': '邪道？我们只是不被所谓的\'正道\'束缚罢了。', '力量': '力量，才是这个世界的真理。' }
  },
  '药铺学徒': {
    greeting: '师兄好！我在学习炼丹呢。',
    topics: { '炼丹': '师父说炼丹需要耐心和悟性。', '材料': '灵草最好找，在竹林里到处都是。', '秘方': '师父有一本祖传秘方，但从不让我们看。' }
  },
  '铁匠学徒': {
  ...
  '江南渔夫': {
    greeting: '客官，要坐船吗？江南水乡可美了！',
    topics: { '水乡': '水乡最美的地方是湖心岛，那里的灵气特别浓郁。', '鱼': '这河里的鱼可不一般，有些鱼肚子里藏着宝贝呢！', '传说': '传说湖底有一座龙宫，里面住着龙王。' }
  },
  '沙漠旅人': {
    greeting: '别看我这样，我可是走遍了大漠的旅人。',
    topics: { '沙漠': '沙漠里最危险的不是沙虫，而是迷路。', '绿洲': '绿洲是沙漠中的天堂，那里的水可以治百病。', '古城': '古城遗迹里据说藏着上古宝物，但也有守墓傀儡。' }
  },
  '冰原猎人': {
    greeting: '这冰天雪地的，你来做什么？',
    topics: { '冰原': '冰原上最可怕的是冰龙，一口寒气能冻住一切。', '极光': '极光出现的时候，雪山之巅会有宝物现世。', '冰狼': '冰狼成群结队，单独遇上必死无疑。' }
  },
  '南疆巫师': {
    greeting: '嘿嘿...你身上有股好闻的味道...',
    topics: { '蛊术': '蛊术是南疆的秘术，用虫子炼蛊，威力无穷。', '毒物': '南疆的毒物可不能乱碰，一不小心就没命了。', '祭坛': '巫蛊祭坛是南疆最神圣的地方，只有最强的巫师才能进入。' }
  },
  '海底蚌精': {
    greeting: '（蚌壳微开，露出一颗珍珠）',
    topics: { '珍珠': '这是我千年来孕育的珍珠，价值连城。', '龙宫': '龙宫在深海最深处，那里有龙王的宝藏。', '海底': '海底的世界比陆地更精彩，但也更危险。' }
  },
  '虚空行者': {
    greeting: '你...能看到我？',
    topics: { '虚空': '虚空是世界的裂缝，这里的时间和空间都是扭曲的。', '帝王': '虚空帝王是虚空的主宰，他已经存在了无数年。', '离开': '想离开虚空？找到虚空王座，那里有回去的路。' }
  },
  '神秘商人': {
    greeting: '嘘...好东西，便宜卖...',
    topics: { '商品': '我这有稀有材料、高级丹药，价格好商量。', '来源': '别问东西从哪来的，问就是捡的。', '秘密': '听说代码空间里有创世者留下的宝藏...' }
  },
  '客栈老板娘': {
    greeting: '客官，住店还是吃饭？',
    topics: { '住店': '住一晚10灵石，可以恢复全部体力。', '消息': '最近来了不少外地人，听说是来寻宝的。', '传闻': '听说竹林深处有块奇怪的石碑，上面写着奇怪的符号。' }
  },
  '药铺老掌柜': {
    greeting: '老朽行医六十载，什么病都能治。',
    topics: { '治病': '中毒了？用解毒丹。受伤了？用回血丹。', '秘方': '我有一祖传秘方，需要龙血和凤泪才能炼制。', '药材': '好药材都在深山老林里，需要有实力才能采集。' }
  },
  '铁匠大师': {
    greeting: '来找我锻造？你的眼光不错！',
    topics: { '锻造': '好武器需要好材料，更需要好手艺。', '强化': '装备强化有风险，失败了可能碎掉。', '传说武器': '传说中有一把混沌剑，需要混沌石才能锻造。' }
  }
};

// ===== NPC好感度系统 =====
const NPC_FAVOR = {
  '村长': { maxFavor: 100, rewards: { 3: '隐藏任务', 5: '遗物线索', 10: '真结局线索' } },
  '药铺老板': { maxFavor: 100, rewards: { 3: '折扣10%', 5: '稀有配方', 10: '仙丹配方' } },
  '铁匠': { maxFavor: 100, rewards: { 3: '折扣10%', 5: '强化成功率+10%', 10: '仙品锻造' } },
  '酒楼老板': { maxFavor: 100, rewards: { 3: '免费休息', 5: '隐藏任务', 10: '绝密情报' } },
  '黑市商人': { maxFavor: 100, rewards: { 3: '稀有物品', 5: '遗物交易', 10: '禁品' } },
  '行商': { maxFavor: 100, rewards: { 3: '折扣15%', 5: '稀有物品', 10: '传说物品' } },
  '神秘老者': { maxFavor: 100, rewards: { 3: '修炼心得', 5: '遗物线索', 10: '创世者秘密' } }
};

function handleNpcFavorCommand(ws, player, npcName) {
  if (!player.npcFavor) player.npcFavor = {};
  const favor = player.npcFavor[npcName] || 0;
  const config = NPC_FAVOR[npcName];
  if (!config) return;
  
  let rewardText = '';
  for (const [threshold, reward] of Object.entries(config.rewards)) {
    const unlocked = favor >= parseInt(threshold);
    rewardText += `║  ${unlocked ? '✅' : '🔒'} 好感${threshold}: ${reward}\n`;
  }
  ...
  sendToClient(ws, {
    ...
    data: {
      title: `${npcName} 好感度`,
      content: `\n╔══════════════════════════════════╗\n║  ${npcName} 好感度\n╠══════════════════════════════════╣\n║  当前好感: ${favor}/${config.maxFavor}\n║  \n${rewardText}╚══════════════════════════════════╝`
    }
  });
}

function handleNpcDialogCommand(ws, player, command) {
  const arg = command.replace('/对话', '').trim();
  if (!arg) {
    const npcs = Object.keys(NPC_DIALOGS);
    sendToClient(ws, { type: 'system', data: { message: `可对话NPC: ${npcs.join(', ')}\n用法: /对话 <NPC名> [话题]` } });
    return;
  }
  ...
  const npc = NPC_DIALOGS[npcName];
  if (!npc) { sendToClient(ws, { type: 'system', data: { message: `找不到NPC: ${npcName}` } }); return; }
  
  let content = `\n╔══════════════════════════════════╗\n║  对话: ${npcName}\n╠══════════════════════════════════╣\n║  "${npc.greeting}"\n║\n`;
  
  if (topic && npc.topics[topic]) {
    content += `║  【${topic}】\n║  "${npc.topics[topic]}"\n║\n`;
  }
  
  content += `║  可聊话题: ${Object.keys(npc.topics).join(', ')}\n╚══════════════════════════════════╝`;
  sendToClient(ws, { type: 'command_response', data: { title: `对话: ${npcName}`, content } });
}

// ===== RANDOM EVENT SYSTEM (随机事件) =====
const RANDOM_EVENTS = [
  { name: '神秘商人', desc: '遇到神秘商人，可以用低价购买稀有物品', effect: p => { p.silver = (p.silver || 0) + 50; return '获得50灵石'; } },
  { name: '灵气爆发', desc: '突然灵气爆发，修炼速度大增', effect: p => { p.exp = (p.exp || 0) + 100; return '获得100经验'; } },
  { name: '道心考验', desc: '面临道心考验', effect: p => { p.daoHeart = Math.min(1000, (p.daoHeart || 500) + 10); return '道心+10'; } },
  { name: '天降奇缘', desc: '天降奇缘，获得稀有材料', effect: p => { if (!p.materials) p.materials = {}; p.materials['天降奇缘'] = (p.materials['天降奇缘'] || 0) + 1; return '获得天降奇缘材料'; } },
  { name: '走火入魔', desc: '修炼走火入魔', effect: p => { p.hp = Math.max(1, (p.hp || 100) - 50); return 'HP-50'; } },
];

function handleRandomEventCommand(ws, player) {
  const event = RANDOM_EVENTS[Math.floor(Math.random() * RANDOM_EVENTS.length)];
  const players = getPlayers();
  const p = players[player.id];
  const result = event.effect(p);
  savePlayers(players);
  sendToClient(ws, { type: 'system', data: { message: `🎲 随机事件: ${event.name}\n${event.desc}\n结果: ${result}` } });
}
...
// ===== LOGGING SYSTEM (日志系统) =====
const LOG_FILE = path.join(DATA_DIR, 'game.log');
function gameLog(type, message) {
  const timestamp = new Date().toISOString();
  const logEntry = `[${timestamp}] [${type}] ${message}\n`;
  try { fs.appendFileSync(LOG_FILE, logEntry); } catch(e) {}
}

// ===== AUTO SAVE & BACKUP (数据备份) =====
function autoBackup() {
  ...
  try {
    ...
    fs.writeFileSync(path.join(backupDir, `players_${timestamp}.json`), JSON.stringify(players, null, 2));
    // Keep only last 10 backups
    const files = fs.readdirSync(backupDir).filter(f => f.startsWith('players_')).sort();
